...
from mcp_common import CircuitBreaker, ironclad_guard, tool_exception_handler
...
@ironclad_guard
@burp_cb
async def burp_send_to_repeater(
    host: str, port: int, use_https: bool, request_b64: str
) -> Dict:
    """Industrial integration: Forward requests to Burp Suite Repeater."""
    burp_api_url = config.get_api_key("burp_api_url") or os.getenv(
...
async def burp_trigger_scan(target_url: str) -> Dict:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"{burp_api_url}/v1/scan", json=payload, headers={"Authorization": api_key}
        ) as resp:
            return await resp.json()
