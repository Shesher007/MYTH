    ...
    try:
        ...
        patterns = {
            ...
            "api_keys": list(
                set(
                    re.findall(
                        r"(?:api|secret|key|token)[_-]?[a-zA-Z0-9]{16,}",
                        "\n".join(decoded_strings),
                        re.I,
                    )
                )
...
def check_open_ports(
    host: str = "127.0.0.1", ports: str = "80,443,22,21,25,3389,8000,8080"
) -> Dict:
    ...
    try:
        for proc in psutil.process_iter(
            ...
            try:
                ...
                if (
                    filter_name
                    and filter_name.lower() not in (pinfo["name"] or "").lower()
                ):
                    continue
                ...
                processes.append(
                    {
                        ...
                        "memory": f"{pinfo['memory_info'].rss / 1024 / 1024:.2f} MB"
        ...
        processes.sort(
            key=lambda x: (
                float(x["memory"].split()[0]) if "N/A" not in x["memory"] else 0
            ),
            reverse=True,
        )

        return {"total_processes": len(processes), "processes": processes[:50]}
    except Exception as e:
        return {"error": str(e)}
    ...
    try:
        ...
        metadata = {
            ...
            "created_iso": time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(stat_info.st_ctime)
            ),
