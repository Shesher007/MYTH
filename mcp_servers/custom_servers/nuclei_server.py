...
async def run_nuclei_scan(
    target: str, templates: List[str] = None, severity: str = None
) -> TitanResponse:
    ...
    if discovered_ports and target in discovered_ports:
        logger.info(
            f"🔱 Intelligence Loop: Found open ports {discovered_ports[target]} for {target}. Optimizing Nuclei scan."
        )
    ...
    if not res["success"]:
        return TitanResponse(
            success=False, data=[], metadata={"error": res.get("stderr")}
        )
