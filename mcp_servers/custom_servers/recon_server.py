...
from mcp_common import (
    MCPUtils, ResourceGuard, QuantumEnricher, ComputeGuard, 
    ironclad_guard, DependencyGuard, tool_exception_handler, logger,
    TitanResponse, NexusState, AccelerationGuard
)
...
async def fast_subdomain_discovery(domain: str) -> TitanResponse:
    ...
    if res["success"]:
        ...
        logger.warning(f"[{cid}] Subfinder failed. Falling back to passive intelligence.")
...
async def rapid_port_scan(target: str) -> TitanResponse:
    ...
    targets = [target]
    if subdomains:
        logger.info(f"🔱 Intelligence Loop: Found {len(subdomains)} subdomains for {target}. Expanding scan.")
    ...
    return TitanResponse(
        success=True,
        data=all_ports,
        metadata={"scanned_targets": len(targets)}
    )
