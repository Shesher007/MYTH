...
LOG_FILE = LOG_DIR / "mcp_fortress.log"
...
class StructLogFormatter(logging.Formatter):
    ...
    def _mask_sensitive(self, text: str) -> str:
        ...
        for pattern in self.SENSITIVE_PATTERNS:
            text = re.sub(pattern, lambda m: f"{m.group(0).split('=')[0]}=****[MASKED]****", text)
        return text
    ...
    def format(self, record):
        ...
        if record.exc_info:
            log_data["exception"] = self._mask_sensitive(self.formatException(record.exc_info))
    ...
    def _init_cache(self):
        ...
        if redis_url:
            try:
                ...
                logger.warning("Redis requested but failed to connect. Falling back to Local.")
        ...
        if cls._pool is None:
            ...
            try:
                ...
                if mem.available < 512 * 1024 * 1024: # < 512MB
                    ...
                    logger.warning(f"Low memory ({mem.available // 1024**2}MB). Scaling ComputePool to {cls._max_workers} workers.")
            except: pass
...
class TitanResponse(BaseModel):
    ...
    hardware_profile: Dict[str, Any] = Field(default_factory=lambda: AccelerationGuard.get_hardware_profile())
...
class PlatformGuard:
    ...
    def safe_subprocess_args(args: List[str]) -> List[str]:
        return [PlatformGuard.map_path(a) if any(c in a for c in ["\\", "/"]) else a for a in args]
            ...
            async def wrapper(*args, **kwargs):
                ...
                if missing or system_bin_missing:
                    ...
                    for sys_bin in system_bin_missing:
                        ...
                        if sb_lower == "nmap":
                            repair_instructions.append("Install Nmap: https://nmap.org/download.html")
                        elif sb_lower == "searchsploit":
                            repair_instructions.append("Install ExploitDB: https://github.com/offensive-security/exploitdb")
                        elif sb_lower == "docker":
                            repair_instructions.append("Install Docker Desktop: https://www.docker.com/products/docker-desktop/")
                        else:
                            repair_instructions.append(f"Install {sys_bin} and add to PATH")
                    
                    error_msg = f"Capability locked. Missing: {', '.join(missing + system_bin_missing)}"
                    logger.error(f"❌ Dependency Failure in {func.__name__}. {error_msg}")
        ...
        for k, v in kwargs.items():
            if isinstance(v, str):
                if len(v) > cls.MAX_STR_LEN:
                    raise ValueError(f"Input '{k}' exceeds safety limit ({cls.MAX_STR_LEN} chars)")
                ...
                raise ValueError(f"Input list '{k}' exceeds safety limit ({cls.MAX_LIST_SIZE} items)")
        return True
...
class ToolResolver:
    ...
    def resolve_command(cmd: List[str]) -> List[str]:
        ...
        KNOWN_TOOLS = {
            "nuclei", "subfinder", "naabu", "httpx", "dnsx", 
            "asnmap", "shuffledns", "katana", "notify", "tlsx",
            "mapcidr", "uncover", "urlfinder", "alterx", "chaos",
            "interactsh-client"
        }
...
class MCPUtils:
    ...
    @staticmethod
    def get_safe_path(path_str: str, base_dir: Optional[Union[str, Path]] = None) -> Path:
    ...
    @staticmethod
    async def run_command_async(cmd: Union[str, List[str]], cwd: Optional[str] = None, timeout: int = 60) -> Dict[str, Any]:
...
# --- Security Primitives ---
def ironclad_guard(func):
    """Decorator for secure execution monitoring."""
    @wraps(func)
    async def wrapper(*args, **kwargs):
        try:
            return await func(*args, **kwargs)
        except Exception as e:
            logger.error(f"Security boundary violation in {func.__name__}: {str(e)}")
            raise
    return wrapper
