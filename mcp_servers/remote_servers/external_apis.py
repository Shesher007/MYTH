...
async def shodan_search(query: str, limit: int = 10) -> Dict:
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            return {"total": data.get("total", 0), "results": results, "query": query}
    ...
    return {
        "error": f"Shodan failed after {max_retries} attempts. Last error: {last_error}"
    }
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=300.0) as client:  # Extended from 30.0
                ...
                return {
                    "url": url,
                    "status": report_data.get("data", {})
                    .get("attributes", {})
                    .get("status"),
                    "stats": report_data.get("data", {})
                    .get("attributes", {})
                    .get("stats"),
                }
        except Exception as e:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:  # Extended from 10.0
            ...
            return {
                "exposed": False,
                "status": "SECURE",
                "message": "No matches found in HIBP database.",
            }
    except Exception as e:
        return {"error": str(e)}
...
async def threat_intelligence_search(
    indicators: List[str], indicator_type: str = "ip"
) -> Dict:
    ...
    try:
        ...
        def _get_cert():
            ...
            with socket.create_connection(
                (hostname, port), timeout=300.0
            ) as sock:  # Extended from 10.0
