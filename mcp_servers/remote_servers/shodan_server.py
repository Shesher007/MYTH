...
class ShodanHostModel(BaseModel):
    ...
    key: Optional[str] = Field(
        None, description="Shodan API Key (default: SHODAN_API_KEY env)"
    )
...
class ShodanSearchModel(BaseModel):
    ...
    facets: Optional[str] = Field(
        None, description="Facets to include (e.g., 'port,os')"
    )
    page: int = Field(1)
...
async def get_shodan_key(provided_key: Optional[str]) -> str:
    ...
    if not key:
        raise ValueError(
            "Shodan API Key not found. Provide it as a tool argument or ensure it is in secrets.yaml."
        )
    return key
...
async def shodan_host_info(ip: str, key: str = None) -> Dict:
    ...
    # Removed unused variable 'start' to fix F841
...
async def shodan_search(
    query: str, facets: str = None, page: int = 1, key: str = None
) -> Dict:
    ...
    params = {"key": api_key, "query": args.query, "page": args.page}
    if args.facets:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params) as resp:
            if resp.status != 200:
                return {
                    "error": f"Shodan Search returned {resp.status}",
