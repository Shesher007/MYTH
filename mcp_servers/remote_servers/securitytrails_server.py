...
async def get_st_key(provided_key: Optional[str]) -> str:
    ...
    if not key:
        raise ValueError("SecurityTrails API Key not found. Ensure it is in secrets.yaml.")
    return key
...
async def securitytrails_dns_history(domain: str, record_type: str = "a", key: str = None) -> Dict:
    """Get historical DNS records (a, aaaa, mx, ns, txt)."""
    api_key = await get_st_key(key)
    url = f"https://api.securitytrails.com/v1/history/{domain}/dns/{record_type.lower()}"
    headers = {"APIKEY": api_key, "Accept": "application/json"}
