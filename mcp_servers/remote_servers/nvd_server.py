            ...
            try:
                async with session.get(url, headers=headers) as resp:
                    ...
                    if resp.status == 403 or resp.status == 401:
                        if api_key:
                            config.invalidate_key("nvd_cve", api_key)
                        continue
                    ...
                    exploit_url = "https://gitlab.com/api/v4/projects/exploit-database%2Fexploit-database/repository/tree?path=exploits&recursive=true"
                    async with session.get(exploit_url) as ex_resp:
                        if ex_resp.status == 200:
                            ex_data = await ex_resp.json()
                            data["exploit_matches"] = [
                                e
                                for e in ex_data
                                if cve_id.lower() in e["name"].lower()
                            ]
                ...
                if attempt == 2:
                    return {"error": f"NVD/ExploitDB failed: {e}"}
