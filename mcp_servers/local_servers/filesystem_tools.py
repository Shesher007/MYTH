...
from mcp_common import (
    MCPUtils, ResourceGuard, QuantumEnricher, ComputeGuard, PlatformGuard,
    UltraPersistence, ironclad_guard, tool_exception_handler, logger,
    TitanResponse
)
...
@ironclad_guard
async def scan_directory(path: str, pattern: str = "*", recursive: bool = True) -> List[Dict]:
        ...
        if args.recursive:
            for root, _, files in os.walk(safe_path):
                for filename in fnmatch.filter(files, args.pattern):
                    ...
                    if len(matches) > 1000: break # Protection
        else:
...
@tool_exception_handler
@ironclad_guard
async def read_file_content(path: str, encoding: str = "utf-8", head: int = 0) -> TitanResponse:
    ...
    async def _stream_read():
        async with aiofiles.open(safe_path, mode='r', encoding=encoding, errors='ignore') as f:
            if head > 0:
                lines = []
                for _ in range(head):
