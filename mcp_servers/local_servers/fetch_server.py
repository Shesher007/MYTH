...
async def recon_fetch(
    url: str, headers: Dict = None, proxy: str = None, ghost_mode: bool = True
) -> Dict:
    ...
    if ghost_mode:
        final_headers.update(
            GhostProtocol.get_stealth_config().get(
                "userAgent", GhostProtocol.get_random_ua()
            )
        )
        if isinstance(final_headers, str):
            final_headers = {
                "User-Agent": final_headers
            }  # Handle potential dict conversion edge

    async with aiohttp.ClientSession(timeout=timeout) as session:
        async with session.get(
            args.url, headers=final_headers, proxy=args.proxy, allow_redirects=True
        ) as resp:
            text = await resp.text()
            server_header = resp.headers.get("Server", "Unknown")

            # Predictive Intel: Broadcast server discovery
            await NexusState.post_intel(
                f"discovery:{url}", {"server": server_header, "status": resp.status}
            )
            return {
                "status": resp.status,
                "server": resp.headers.get("Server", "Unknown"),
                "security_headers": {
                    k: v for k, v in resp.headers.items() if "security" in k.lower()
                },
