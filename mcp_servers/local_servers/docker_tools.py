...
async def live_container_forensics(container_id: str) -> Dict:
    ...
    try:
        ...
        return {
            "error": f"Exec failed: {str(e)}",
            "note": "Is the container running a supported shell/OS?",
        }
...
async def docker_cleanup() -> Dict:
    ...
    def _prune():
        ...
        client.images.prune()
        return {
            "containers_deleted": len(c.get("ContainersDeleted", [])),
            "space_reclaimed": c.get("SpaceReclaimed", 0),
        }
