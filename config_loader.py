...
class PromptsConfig(BaseModel):
    ...
    def get_full_system_prompt(self, category: str = "core") -> str:
        ...
        # ===================================================================
        directives = "\n".join(
            [f"{i + 1}. {d}" for i, d in enumerate(self.core_directives)]
        )
        ...
        if category in ["complex", "security"] and "offensive" in (
            self.few_shot_examples or {}
        ):
...
class ConfigurationManager:
    ...
    def load_config(self, path: str = "governance/agent_manifest.yaml"):
        try:
            ...
            logger.info(
                f"✅ Peak Industrial Configuration Loaded: {self.config.identity.name} v{self.config.identity.version} [{self.config.runtime.environment}]"
            )

            if self.config.observability.logging_level:
                level = getattr(
                    logging,
                    self.config.observability.logging_level.upper(),
                    logging.INFO,
                )
