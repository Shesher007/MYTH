...
# Force Proactor loop for Windows compatibility (Required for subprocess pipes & Cloudflare)
...
from myth_utils.paths import get_resource_path, get_app_data_path
log_formatter = logging.Formatter('%(asctime)s [%(levelname)s] [%(name)s] %(message)s')

# 1. File Handler (Full Detail Persistence)
log_file_path = get_app_data_path(f"logs/{agent_config.identity.name.lower()}_system.log")
...
logging.basicConfig(
    level=logging.INFO,
    handlers=[file_handler, stream_handler, ui_bridge_handler]
)
...
class ConfigMeta(type):
    ...
    @property
    def NVIDIA_KEY(cls): return config.get_api_key("nvidia")
    
    @property
    def MISTRAL_KEY(cls): return config.get_api_key("mistral")
...
class ReliabilityLayer:
    ...
    def retry_with_backoff(retries=Config.MAX_RETRIES, monitor_tag="LLM"):
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                ...
                logger.error(f"❌ [{monitor_tag}] CRITICAL FAILURE after {retries} attempts.")
    ...
    def normalize_tool_output(output: Any) -> str:
        ...
        """
        if output is None: return "Operation completed with no output."
        s = str(output)
...
SETTINGS_LOCK = threading.Lock() # Industrial Grade: Prevent Read-Modify-Write race conditions
...
async def _ensure_playwright_browsers():
    ...
    try:
        ...
        logger.warning("⚠️ [INIT] Playwright library not found. Browser tools will fail.")
        return
    ...
    try:
        async with async_playwright() as p:
            try:
                ...
                logger.info("🛠️ [INIT] Playwright Chromium missing or outdated. Triggering auto-installation...")
                notify_system("INFO", "Browser Engine", "Downloading Chromium for web tools. This may take a few minutes on first run...")
                ...
                try:
                    ...
                    logger.warning("⚠️ [INIT] Using fallback Playwright install method (may fail in frozen mode)")
                ...
                if process.returncode == 0:
                    ...
                    notify_system("SUCCESS", "Browser Engine", "Chromium browser engine installed and ready.")
                else:
                    error_msg = stderr.decode() if stderr else stdout.decode() if stdout else "Unknown error"
                    logger.error(f"❌ [INIT] Playwright installation failed: {error_msg}")
                    notify_system("WARNING", "Browser Engine", f"Chromium installation failed. Browser tools may not work: {error_msg[:200]}")
    except Exception as e:
...
async def initialize_system_async():
    ...
    REGISTRY["prompts"]["complex"] = agent_config.prompts.get_full_system_prompt(category="complex")
    REGISTRY["prompts"]["simple"] = agent_config.prompts.get_full_system_prompt(category="simple")
    
    # Dependency Guard (First-Run Logic)
    await _inject_sidecar_paths()
    asyncio.create_task(_ensure_playwright_browsers()) # Run in background to avoid blocking bootstrap
    ...
    notify_system("INFO", "Neural Matrix Initializing", "Synchronizing industrial prompts and hydrating model registry...")
    ...
    try:
        ...
        from rag_system import (
            VectorStoreManager, FileUploader, ImageProcessor, FolderProcessor, RAGChain
        )
        from langchain_core.tools import Tool
        notify_system("SUCCESS", "Neural Dependencies Loaded", "Industrial AI libraries and RAG core components linked.")
    ...
    def get_provider(m_name: str) -> str:
        ...
        if any(p in m for p in ["mistral", "codestral", "voxtral"]): return "mistral"
    ...
    async def _provision_node(node, m_name):
        try:
            ...
            logger.info(f"✅ [INIT] Node '{node}' mapped to {provider} ({m_name}) | Temp: {profile.temperature}")
    ...
    provision_tasks = [_provision_node(node, m_name) for node, m_name in nodes_to_provision.items()]
    ...
    notify_system("SUCCESS", "Neural Matrix Hydrated", f"Model registry active. Bindings: {', '.join(active_m)}")



    if not REGISTRY["models"]["fallback"]:
        logger.critical("❌ [INIT] NO LLM CORE AVAILABLE. System will be non-functional.")
    ...
    async def init_rag():
        ...
        try:
            ...
            async def init_rag_subsystems():
                REGISTRY["file_uploader"] = FileUploader(upload_dir=upload_dir, vector_store_manager=REGISTRY["vector_store"])
                REGISTRY["image_processor"] = ImageProcessor(nvidia_api_key=Config.NVIDIA_KEY, vision_model=Config.IMAGE_MODEL)
                
                try:
                    from rag_system.audio_processor import AudioProcessor
                    REGISTRY["audio_processor"] = AudioProcessor(mistral_api_key=Config.MISTRAL_KEY)
                except ImportError:
                    logger.warning("⚠️ audio_processor.py not found. Skipping.")

                REGISTRY["folder_processor"] = FolderProcessor(
                    max_workers=Config.MAX_PARALLEL_TASKS, vector_store=REGISTRY["vector_store"]
                )
    ...
    try:
        ...
        try:
            ...
            logger.error("⚠️ [INIT] Tool discovery timed out (30s). Booting with partial capability.")
            combined_tools = await get_all_tools(force_refresh=False) # Fallback to whatever is cached
        ...
        if REGISTRY["file_uploader"]:
            @tool
            async def upload_and_process_file(file_path: str, collection: str = "security_docs"):
                """Ingest a specific file on disk into the RAG knowledge base for analysis."""
                return await REGISTRY["file_uploader"].process_existing_file(file_path, collection)
        ...
        if REGISTRY["folder_processor"]:
            @tool
            async def upload_and_process_folder(folder_path: str, collection: str = "folder_docs"):
                """Recursively ingest all supported files from a folder into the RAG system."""
                return await REGISTRY["folder_processor"].aprocess_folder(folder_path, collection)
        ...
        if REGISTRY["rag_chain"]:
            ...
            @tool
            async def query_knowledge_base(query: str, collection: str = "security_docs"):
                """Search the neural core (indexed files) for security metrics, code, or logs."""
                try:
                    logger.info(f"🔍 [RAG] Searching collection '{collection}' for: {query}")
                    return await REGISTRY["rag_chain"].query_with_sources(collection_name=collection, query=query)
                except Exception as e:
                    logger.error(f"❌ [RAG] Search failed: {e}")
                    return {"answer": f"Search operation failed: {str(e)}", "sources": []}
            ...
            async def vuln_exploit_database_lookup(cve_id: str):
                ...
                try:
                    ...
                    res = await REGISTRY["rag_chain"].query_with_sources(collection_name="pentest_kb", query=f"Details and exploit proof-of-concept for {cve_id}")
                    return json.dumps(res, indent=2)
                except Exception as e:
                    return json.dumps({"error": f"CVE database lookup failed: {str(e)}"}, indent=2)
        ...
        all_candidate_tools = combined_tools + rag_tools + rag_query_tools + web_agent_tools
        ...
        for t in all_candidate_tools:
            ...
            if not agent_config.capabilities.shell_access_enabled:
                if any(k in t_name for k in ['execute', 'shell', 'bash', 'powershell', 'python_script', 'system_info', 'list_directory']):
                    continue
            
            # 2. Web Search Control
            if not agent_config.capabilities.web_search_enabled:
                if any(k in t_name for k in ['search', 'google', 'bing', 'duckduckgo', 'wikipedia', 'crawl', 'scrape']):
                    continue
...
def extract_mission_context(messages: List[BaseMessage]) -> str:
    ...
    for msg in reversed(messages): # Check entire visible history for context
        ...
        if "[System Info:" in content:
             ...
             logger.debug(f"📍 [DEBUG] Extracted System Info.")
             
    # De-duplicate to keep prompt clean
    unique_segments = list(dict.fromkeys(context_segments))
    res = "\n".join(unique_segments) if unique_segments else "No specific attachments detected in this stream."
    return res

def prune_chat_history(messages: List[BaseMessage], max_tokens: int = 100000) -> List[BaseMessage]:
    """
    ...
    if current_chars < max_tokens * 3:
        ...
        messages_valid = True
        for m in messages:
            if isinstance(m, ToolMessage) and not any(isinstance(prev, AIMessage) for prev in messages[:messages.index(m)]):
    ...
    for i, b in enumerate(blocks):
        if isinstance(b[0], AIMessage) and getattr(b[0], "tool_calls", None):
            ...
            if num_calls != num_res and i < len(blocks) - 1:
                ...
                fixed_ai = AIMessage(content=b[0].content, additional_kwargs=b[0].additional_kwargs)
...
def heuristic_fast_path(input_text: str) -> Optional[str]:
    ...
    if word_count <= 5:
        casual_patterns = [
            ...
            "ok", "okay", "cool", "nice", "great", "good", "sure",
    ...
    # =========================================================================
    identity_patterns = [
        "who are you", "what are you", "who is myth", "what is myth",
        "who created you", "who made you", "your name", "your creator",
        "status", "health", "ready", "are you there", "you there", "ping"
    ]
    ...
    # =========================================================================
    config_patterns = ["architecture", "mode", "settings", "config", "api status", "model pulse", "debug info"]
    ...
    # Capability queries
    capability_patterns = [
        "what can you do", "what are your capabilities", "list capabilities",
        "show tools", "available tools", "help", "show commands"
    ]
    ...
    if any(text.startswith(s) for s in explanation_starts):
        target_indicators = ["scan", "attack", "hack", "target", "192.", "10.", "localhost", "save", "file"]
    ...
    # =========================================================================
    actions = ["clear history", "reset session", "stop current mission", "get latest alerts", "mission summary"]
    ...
    # =========================================================================
    action_keywords = [
        "save", "create", "generate", "download", 
        "scan", "attack", "hack", "exploit", "inject", "enumerate", 
        "brute", "crack", "dump", "exfiltrate", "pivot", "bypass"
    ]
...
async def router_node(state: ChatState):
    ...
    if any(re.search(p, last_input_lower) for p in complex_patterns):
        ...
        technical_markers = ["code", "script", "python", "bash", "file", "txt", "exploit", "payload"]
        
        if any(n in last_input_lower for n in creative_nouns) and not any(t in last_input_lower for t in technical_markers):
    ...
    @ReliabilityLayer.retry_with_backoff(retries=Config.MAX_RETRIES, monitor_tag="ROUTER")
    async def _call():
        ...
        model_id = getattr(router, 'model', getattr(router, 'model_name', 'router_llm'))
        response = await asyncio.wait_for(router.ainvoke(prompt, config={"run_name": "router_llm"}), timeout=Config.INIT_TIMEOUT)
        return str(response.content).strip().upper() if response and hasattr(response, 'content') else "SIMPLE"
...
async def blueprint_node(state: ChatState):
    ...
    if is_replan:
        logger.info(f"🔄 [BLUEPRINT] Re-planning triggered (iteration {current_loop + 1}): {replan_reason}")

    @ReliabilityLayer.retry_with_backoff(retries=Config.MAX_RETRIES, monitor_tag="BLUEPRINT")
    async def _call():
        model = get_model("blueprint")
        if not model: return "Execute standard protocol.", "1. Analyze request.", "utilities"
        ...
        search_task = asyncio.create_task(search_tools(user_query, max_results=15)) # Increased results for better coverage
        ...
        if matched_tools:
            ...
            for t in matched_tools:
                ...
                args = t.get("args", {})
                if not isinstance(args, dict): args = {"properties": {}}
                props_json = json.dumps(args.get("properties", {}))
                tool_info.append(f"- {t['name']}: {t['description']}\n  SCHEMA: {props_json}")
        ...
        if is_replan and replan_reason:
            ...
            intensity_directive = "PATCH existing plan" if plan_intensity == "PATCH" else "RE-PLAN entire mission"
            replan_section = f"""
...
"""
        ...
        response = await asyncio.wait_for(model.ainvoke(prompt, config={"run_name": "blueprint_llm"}), timeout=Config.TOOL_TIMEOUT)
        content = str(response.content) if response and hasattr(response, "content") else ""
        
        # 3. ROBUST EXTRACTION & VALIDATION
        import re
        strat_match = re.search(r"<STRATEGY>(.*?)</STRATEGY>", content, re.DOTALL | re.IGNORECASE)
        plan_match = re.search(r"<PLAN>(.*?)</PLAN>", content, re.DOTALL | re.IGNORECASE)
        caps_match = re.search(r"<CAPABILITIES>(.*?)</CAPABILITIES>", content, re.DOTALL | re.IGNORECASE)
        
        strategy = strat_match.group(1).strip() if strat_match else "Proceed with standard analysis."
        ...
        if valid_tool_names:
            ...
            unknown_tools = [t for t in planned_tools if t not in valid_tool_names and t not in ["create_advanced_file", "atomic_write_file"]]
            
            if unknown_tools:
                logger.warning(f"⚠️ [BLUEPRINT] Detected potentially hallucinated tools: {unknown_tools}")
    ...
    try:
        ...
        if any(k in user_query for k in ["save", "write", "create", "file", "download"]):
             # Check if we already produced a file artifact in this session
             already_generated = False
             for msg in reversed(state["messages"]):
                 if isinstance(msg, ToolMessage):
                     content_lower = msg.content.lower()
                     if "hash" in content_lower or "sha256" in content_lower or "successfully generated" in content_lower:
                         already_generated = True
                         break
             
             if not already_generated and "create_advanced_file" not in plan and "atomic_write_file" not in plan:
                ...
                if "utilities" not in capabilities: capabilities += ", utilities"
        ...
        return {"tactical_plan": "1. Analyze request directly.", "strategy_manifest": "Standard"}
...
async def executor_node(state: ChatState):
    ...
    try:
        await asyncio.wait_for(REGISTRY["is_ready_event"].wait(), timeout=Config.INIT_TIMEOUT)
    except asyncio.TimeoutError:
        return {"messages": [AIMessage(content="System core initialization timed out.")]}
    ...
    if classification == "COMPLEX":
        from tools import get_tools_by_category, TOOL_CATEGORIES
        caps_str = state.get("mission_context", "utilities")
        import re
        cleaned_caps = re.sub(r"[\[\]]", "", caps_str).split(",")
        required_categories = [c.strip().lower() for c in cleaned_caps if c.strip().lower() in TOOL_CATEGORIES]
        if "utilities" not in required_categories: required_categories.append("utilities")
        
        logger.info(f"🎯 [EXECUTOR] Deterministic Tool Binding (Industrial Global): {required_categories}")
        active_tools = await get_tools_by_category(required_categories, provided_tools=REGISTRY["tools"])
    # FAILSAFE: If SIMPLE but contains action keywords, bind essential utilities anyway
    elif any(k in str(state["messages"][-1].content).lower() for k in ["save", "write", "create", "file", ".txt"]):
        from tools import get_tools_by_category
        logger.info("⚠️ [EXECUTOR] Simple mode action detected. Applying utility binding failsafe.")
        active_tools = await get_tools_by_category(["utilities"], provided_tools=REGISTRY["tools"])
        
    # INDUSTRIAL ENHANCEMENT: Dynamic Strategic Binding (Multi-Layer Extraction)
    if classification == "COMPLEX":
        from tools import get_tool_by_name, get_all_tools
        import re
    ...
    if classification == "COMPLEX" and plan:
        ...
        for line in plan_lines:
            matched_tool = None
            for t in (state.get("tools", []) or []): # Reference tools if available in state
                if t.name in line:
                    matched_tool = t.name
                    break
            ...
            if not matched_tool:
                for t_name in seen_tools: # Check against what we already know succeeded
                    if t_name in line:
                        matched_tool = t_name
                        break
            ...
            if matched_tool:
                if matched_tool not in seen_tools:
                    ...
                    logger.debug(f"✂️ [EXECUTOR] Pruning already-completed tool from plan: {matched_tool}")
            else:
                pruned_plan_lines.append(line)
    ...
    @ReliabilityLayer.retry_with_backoff(retries=Config.MAX_RETRIES, monitor_tag="EXECUTOR")
    async def _exec():
        model_id = getattr(model, 'model', getattr(model, 'model_name', 'executor_llm'))
        response = await model.ainvoke(msgs, config={"run_name": "executor_llm", "tags": [model_id], "metadata": {"model_id": model_id}})
    ...
    try:
        ...
        if hasattr(response, "content") and isinstance(response.content, str):
            ...
            if not response.content.strip() and not response.tool_calls:
                 logger.warning("🛡️ [EXECUTOR] Refusal detected. Triggering Industrial Re-Focus...")
                 ...
                 try:
                     model_id = getattr(model, 'model', getattr(model, 'model_name', 'executor_llm'))
                     response = await model.ainvoke(refocus_msgs, config={"run_name": "executor_llm_refocus", "tags": [model_id, "refocus"]})
                     if hasattr(response, "content") and isinstance(response.content, str):
                         response.content = SovereignSanitizer.clean_text(response.content)
...
async def tool_node(state: ChatState):
    ...
    async def run_tool(tc):
        ...
        for msg in reversed(messages[-6:]): # Look back at last 3 turns
            if isinstance(msg, ToolMessage):
                meta = msg.additional_kwargs.get("_myth_meta", {})
                if meta.get("tool_name") == name and meta.get("args") == args and meta.get("success"):
                    logger.info(f"🔄 [TOOL] Deduplication Triggered: '{name}' already succeeded. Returning cached result.")
                    return msg
        ...
        if tool_hash in (state.get("denied_hashes", []) or []):
            ...
            notify_system("WARNING", "Tactical Execution Denied", f"Operator policy blocked: {name}")
        ...
        if not target: 
            ...
            notify_system("ERROR", "Registry Failure", f"Component '{name}' not found in tactical arsenal.")

            return ToolMessage(
                content=f"Tool '{name}' not found in registry.", 
                tool_call_id=tc["id"], 
                name=name,
                additional_kwargs={"_myth_meta": {"success": False, "error": "Not Found"}}
            )
        ...
        for attempt in range(max_tool_retries + 1):
            ...
            try:
                ...
                if attempt > 0 and isinstance(args, dict):
                    logger.info(f"🔄 [RESILIENCE] Attempting auto-sanitization for {name} (Attempt {attempt})")
                    for k, v in args.items():
                        if isinstance(v, str):
                            # Strip common protocol prefixes
                            if any(p in k.lower() for p in ["target", "host", "url", "domain"]):
                                if v.startswith("http"):
                                    from urllib.parse import urlparse
                                    domain = urlparse(v).netloc or v.replace("http://", "").replace("https://", "").split("/")[0]
                                    args[k] = domain
                ...
                if hasattr(target, 'ainvoke'): 
                    res = await asyncio.wait_for(target.ainvoke(args), timeout=Config.TOOL_TIMEOUT)
                else: 
                ...
                tool_msg = ToolMessage(content=normalized_res, tool_call_id=tc["id"], name=name)
                tool_msg.additional_kwargs["_myth_meta"] = {
                    "tool_name": name, "args": args, "execution_time_ms": execution_time,
                ...
                execution_time = int((time.time() - start_time) * 1000)
                logger.warning(f"⚠️ [TOOL] {name} failed (Attempt {attempt+1}/{max_tool_retries+1}): {e}")
                ...
                # Final Failure
                tool_msg = ToolMessage(content=f"Error: {str(e)}", tool_call_id=tc["id"], name=name)
                tool_msg.additional_kwargs["_myth_meta"] = {
                    "tool_name": name, "args": args, "execution_time_ms": execution_time,
...
async def reflection_node(state: ChatState):
        ...
        for msg in reversed(state["messages"][-10:]):  # Last 10 messages
            if isinstance(msg, ToolMessage):
                ...
                # Errors with specific patterns are valuable
                    ...
                    if any(p in error_str for p in ["permission", "firewall", "blocked", "refused", "timeout"]):
                        has_learning_value = True  # Security-relevant failure
                
                # Successful tool with discovery indicators
                elif meta.get("success"):
                    output_lower = output.lower()
                    discovery_indicators = ["found", "discovered", "detected", "vulnerable", "open", "exposed", "warning"]
                    if any(ind in output_lower for ind in discovery_indicators):
                        has_learning_value = True
                
                if not has_learning_value:
                    logger.debug(f"[REFLECTION] Skipping low-learning-value result: {tool_name}")
                    continue
                ...
                if meta.get("success"):
                    ...
                    if "timeout" in error_str:
                        success_score = 0.4  # Timeouts indicate target exists but is slow/protected
    ...
    if len(recent_tool_sigs) >= 3 and all(s == recent_tool_sigs[0] for s in recent_tool_sigs[:3]):
        logger.warning("🔄 [REFLECTION] Infinite Loop Detected (Identical tool calls). Forcing HARD REPLAN.")
        return {
            "needs_replan": True,
    ...
    if current_loop >= MAX_LOOPS:
        logger.warning(f"🛑 [REFLECTION] Max loops ({MAX_LOOPS}) reached. Terminating recursion.")
    ...
    if auditor:
        try:
            ...
            last_tool_output = state["messages"][-1].content if isinstance(state["messages"][-1], ToolMessage) else "No tool output."
            ...
            response = await asyncio.wait_for(auditor.ainvoke(prompt, config={"run_name": "reflection_llm"}), timeout=Config.INIT_TIMEOUT)
            ...
            logger.warning("🌐 [REFLECTION] Connection reset by host. Skipping audit for stability.")
            return {"needs_replan": False, "replan_reason": None}
        except Exception as e:
             if "10054" in str(e):
                 logger.warning("🌐 [REFLECTION] WinError 10054 detected. Skipping audit.")
                 return {"needs_replan": False, "replan_reason": None}
             logger.warning(f"⚠️ [REFLECTION] Audit failed: {e}. Defaulting to standard flow.")
    ...
    for msg in reversed(state["messages"][-3:]):
        if isinstance(msg, ToolMessage):
            ...
            if meta.get("tool_name") == "create_advanced_file" and meta.get("success"):
                logger.info("🎯 [REFLECTION] Mission Objective achieved (File Persisted). Terminating chain.")
                return {"needs_replan": False, "replan_reason": "Task completed successfully."}
...
def route_after_router(state: ChatState):
    ...
    try:
        res = "blueprint_node" if state.get("classification") == "COMPLEX" else "executor_node"
...
def route_after_reflection(state: ChatState):
    ...
    try:
        if state.get("needs_replan", False):
            ...
            res = "blueprint_node"
            logger.info(f"[ROUTING] Adaptive re-routing to {res.upper()} (Intensity: {intensity})")
        else:
            res = "executor_node"
        latency = (time.perf_counter() - start_time) * 1000
...
builder.add_conditional_edges("executor_node", route_after_executor, ["tools", "__end__"])
...
builder.add_conditional_edges("reflection_node", route_after_reflection, ["blueprint_node", "executor_node"])
