            ...
            socket.onmessage = (event) => {
                try {
                    ...
                    if (data.type === 'SYSTEM_TELEMETRY_V2') {
                        ...
                        if (s.vpn) {
                            setVpnStatus({
                                connected: s.vpn.connected,
                                active_node: s.vpn.active_node,
                                throughput_tx: s.vpn.throughput_tx,
                                throughput_rx: s.vpn.throughput_rx,
                                uptime: s.vpn.uptime,
                                ip_virtual: s.vpn.ip_virtual,
                                encryption: s.vpn.encryption
                            });
                            if (s.vpn.nodes) setVpnNodes(s.vpn.nodes);
                        }
