...
const VERIFICATION_PUB_KEY_HEX: &str =
    "70971817cba408b7f80d645f188600f401f3ab0240be4b3ddeed688b0f22f874";

/// License server base URL
const LICENSE_SERVER_URL: &str = "https://myth-auth-engine.shesher0llms.workers.dev";
...
pub async fn activate_license(key: String) -> Result<serde_json::Value, String> {
    ...
    if activation.success {
        if let Some(cert) = &activation.certificate {
            // --- Proper Security: Immediate Verification ---
            // Verify the server's work against our local Public Key before committing to disk.
            match verify_signature(cert) {
                Ok(true) => {
                    save_license(cert)?;
                    Ok(serde_json::json!({
                ...
                Ok(false) => {
                    Err("Security Violation: Server signature verification failed locally. Possible MITM or key mismatch.".to_string())
                },
                Err(e) => {
                    Err(format!("Local verification error: {}", e))
                }
            }
        } else {
...
mod tests {
    ...
    fn test_signature_verification() {
        // This is the public key from identity.yaml
        let pub_key_hex = "70971817cba408b7f80d645f188600f401f3ab0240be4b3ddeed688b0f22f874";
        
        // Mock certificate
        let cert = LicenseCertificate {
            activation_key: "MYTH-TEST-KEY".to_string(),
            device_fingerprint: "TEST-HWID".to_string(),
            license_tier: "Pro-Elite".to_string(),
            expiration: None,
            issued_at: "2026-02-20T10:00:00Z".to_string(),
            signature: "M+iO5e6l+O/Y5z4z3X9O5e6l+O/Y5z4z3X9O5e6l+O/Y5z4z3X9O5e6l+O/Y5z4z3X9O5e6l+O/Y5z4z3X9O5A==".to_string(), 
        };

        match verify_signature(&cert) {
            Ok(_) => println!("Signature check function is operational."),
            Err(e) => panic!("Signature check function errored: {}", e),
        }
    }
