...
class Scan(object):
    ...
    def write_nmaprun_open(self, writer):
        ...
        if self.start_date is not None:
            ...
            attrs["startstr"] = self.start_date.strftime("%a %b %d %H:%M:%S %Y")
    ...
    def nmaprun_to_dom_fragment(self, document):
        ...
        if self.start_date is not None:
            elem.setAttribute("start", "%d" % time.mktime(self.start_date.timetuple()))
            elem.setAttribute(
                "startstr", self.start_date.strftime("%a %b %d %H:%M:%S %Y")
            )
        if self.version is not None:
...
class Host(object):
    ...
    def extraports_string(self):
        ...
        return ", ".join(["%d %s ports" % (count, state) for (count, state) in list])
...
def format_banner(scan):
    ...
    if scan.start_date is not None:
        parts.append("initiated %s" % scan.start_date.strftime("%a %b %d %H:%M:%S %Y"))
    if scan.args is not None:
        parts.append("as: %s" % scan.args)
    return " ".join(parts)


def print_script_result_diffs_text(
    title, script_results_a, script_results_b, script_result_diffs, f=sys.stdout
):
    table = Table("*")
...
def script_result_diffs_to_dom_fragment(
    elem, script_results_a, script_results_b, script_result_diffs, document
):
...
class ScanDiffText(ScanDiff):
    ...
    def output_pre_scripts(self, pre_script_result_diffs):
        print_script_result_diffs_text(
            "Pre-scan script results",
            self.scan_a.pre_script_results,
            self.scan_b.pre_script_results,
            pre_script_result_diffs,
            self.f,
        )
...
class HostDiff(object):
    ...
    def diff(self):
        ...
        if set(self.host_a.addresses) != set(self.host_b.addresses) or set(
            self.host_a.hostnames
        ) != set(self.host_b.hostnames):
            self.id_changed = True
            self.cost += 1

        all_specs = list(
            set(self.host_a.ports.keys()).union(set(self.host_b.ports.keys()))
        )
        all_specs.sort()
        ...
        extraports_a = tuple(
            (count, state) for (state, count) in self.host_a.extraports.items()
        )
        extraports_b = tuple(
            (count, state) for (state, count) in self.host_b.extraports.items()
        )
    ...
    def include_diff(self, diff):
        ...
        if self.host_a.is_extraports(diff.port_a.state) and self.host_b.is_extraports(
            diff.port_b.state
        ):
            return False
        elif verbose:
        ...
        if self.os_changed or verbose:
            ...
            for op, i1, i2, j1, j2 in self.os_diffs:
                if op == "replace" or op == "delete":
                    ...
                    for i in range(i1, i2):
                        a_elem.appendChild(
                            host_a.os_to_dom_fragment(document, host_a.os[i])
                        )
                ...
                if op == "replace" or op == "insert":
                    ...
                    for i in range(j1, j2):
                        b_elem.appendChild(
                            host_b.os_to_dom_fragment(document, host_b.os[i])
                        )
...
class PortDiff(object):
    ...
    def to_dom_fragment(self, document):
        ...
        if (
            self.port_a.spec == self.port_b.spec
            and self.port_a.state == self.port_b.state
        ):
...
class NmapContentHandler(xml.sax.handler.ContentHandler):
    ...
    def _start_status(self, name, attrs):
        ...
        if state is None:
            warn(
                '%s element of host %s is missing the "state" attribute; '
    ...
    def _start_address(self, name, attrs):
        ...
        if addr is None:
            warn(
                '%s element of host %s is missing the "addr" '
                "attribute; skipping." % (name, self.current_host.format_name())
            )
            return
    ...
    def _start_hostname(self, name, attrs):
        ...
        if hostname is None:
            warn(
                '%s element of host %s is missing the "name" '
                "attribute; skipping." % (name, self.current_host.format_name())
            )
            return
    ...
    def _start_extraports(self, name, attrs):
        ...
        if state is None:
            warn(
                '%s element of host %s is missing the "state" '
        ...
        if state in self.current_host.extraports:
            warn(
                'Duplicate extraports state "%s" in host %s.'
        ...
        if count is None:
            warn(
                '%s element of host %s is missing the "count" '
                "attribute; assuming 0." % (name, self.current_host.format_name())
            )
            count = 0
        else:
            try:
                ...
                warn(
                    'Can\'t convert extraports count "%s" '
    ...
    def _start_port(self, name, attrs):
        ...
        if portid_str is None:
            warn(
                '%s element of host %s missing the "portid" '
                "attribute; skipping." % (name, self.current_host.format_name())
            )
            return
        try:
            ...
            warn(
                'Can\'t convert portid "%s" to an integer '
        ...
        if protocol is None:
            warn(
                '%s element of host %s missing the "protocol" '
                "attribute; skipping." % (name, self.current_host.format_name())
            )
            return
    ...
    def _start_state(self, name, attrs):
        ...
        if "state" not in attrs:
            warn(
                '%s element of port %s is missing the "state" '
                'attribute; assuming "unknown".'
    ...
    def _start_script(self, name, attrs):
        ...
        if self.parent_element() == "prescript":
            ...
            warn(
                "%s element not inside prescript, postscript, hostscript, "
