...
from myth_config import config as sovereign_config
    ...
    def _find_binary(self, tool_name: str) -> str:
        ...
        try:
            ...
            if possible_sidecar_dir.exists():
                common_paths.append(
                    str(
                        possible_sidecar_dir / f"{tool_name}-x86_64-pc-windows-msvc.exe"
                    )
                )
        except Exception:
            pass
        ...
        if self.is_windows:
            # Windows paths
            exe_name = (
                f"{tool_name}.exe" if not tool_name.endswith(".exe") else tool_name
            )
            common_paths = [
...
def run_command_sync(cmd: List[str], timeout: int = 300) -> Dict[str, Any]:
    """
    Run command synchronously for legacy-style tools.
    """
    try:
        cmd = [str(c) if isinstance(c, Path) else c for c in cmd]
        start_time = time.time()
...
async def nuclei_scan(
    ...
    try:
        ...
        return json.dumps({"error": f"Nuclei scan error: {str(e)}"}, indent=2)
...
async def subfinder_scan(
    ...
    try:
        ...
        return json.dumps({"error": f"Subfinder scan error: {str(e)}"}, indent=2)
...
async def httpx_scan(
    ...
    try:
        if not config.binary_paths.get("httpx"):
            return format_industrial_result(
                "httpx_scan", "Error", error="HTTPx not installed"
            )
        ...
        response_data = {
            ...
            "technologies": dict(
                sorted(technologies.items(), key=lambda x: x[1], reverse=True)
            ),
            "sample": responses[:5],
            "command": " ".join(cmd),
            "execution_time": result.get("execution_time", 0),
...
async def dnsx_scan(
    ...
    try:
        ...
        if Path(domains).exists():
            args.extend(["-l", domains])
        else:
            args.extend(["-d", domains])

        if query_type:
            args.extend(["-query", query_type])
        if resolver:
...
@tool
async def asnmap_scan(target: str, org: str = "", verify: bool = True) -> str:
...
async def industrial_parallel_discoverer(target: str) -> str:
    ...
    try:
        ...
        from tools.recon.discovery import httpx_scan, naabu_scan, subfinder_scan
        ...
        targets_str = ",".join(
            subdomains[:20]
        )  # Limit to 20 for speed in parallel pass
        ...
        stages = [
            {
                ...
                "status": "COMPLETED",
                "findings": len(subdomains),
            },
            {
                "stage": "Port Scanning",
                "status": "COMPLETED",
                "findings": len(naabu_data.get("raw_data", {}).get("open_ports", [])),
            },
            {
                "stage": "HTTP Probing",
                "status": "COMPLETED",
                "findings": len(httpx_data.get("raw_data", {}).get("live_hosts", [])),
            },
        ]
        ...
        return format_industrial_result(
            ...
            raw_data={
                "target": target,
                "stages": stages,
                "discovery_chain": "Subfinder->Naabu->HTTPx",
            },
        ...
        return format_industrial_result(
            "industrial_parallel_discoverer", "Error", error=str(e)
        )
...
async def recon_genesis_orchestrator() -> str:
    ...
    try:
        ...
        preflight_chain = [
            {
                "monitor": "active_recon_genesis",
                "status": "PASSED" if "ONLINE" in mon1 else "FAILED",
            },
            {
                "monitor": "passive_integrity_check",
                "status": "PASSED" if "Stable" in mon2 else "FAILED",
            },
        ]
        ...
        return format_industrial_result(
            "recon_genesis_orchestrator", "Error", error=str(e)
        )
...
async def alterx_generate(
    base_domain: str, wordlist: str = "", patterns: str = "", permutations: int = 10000
) -> str:
...
async def tlsx_scan(
    ...
    try:
        ...
        if Path(targets).exists():
            args.extend(["-l", targets])
        else:
            args.extend(["-u", targets])
...
async def mapcidr_operations(
    ...
    try:
        ...
        if operation == "list":
            ...
            args.append("-count")
        elif operation == "subnet" and subnet:
            args.extend(["-sb", subnet])
        elif operation == "filter" and filter_ips:
            args.extend(["-filter", filter_ips])
...
@tool
async def alterx_pattern_generator(
    base_domain: str, pattern_type: str = "aggressive"
) -> str:
    """
    ...
    try:
        if not config.binary_paths.get("alterx"):
            return format_industrial_result(
                "alterx_pattern_generator", "Error", error="AlterX not installed"
            )
        ...
        return format_industrial_result(
            "alterx_pattern_generator", "Error", error=str(e)
        )
    ...
    try:
        ...
        if use_alterx and config.binary_paths.get("alterx"):
            ...
            alterx_output = await alterx_generate(
                base_domain=domain, permutations=permutations
            )
        ...
        if validate_dns and all_subdomains_list and config.binary_paths.get("dnsx"):
            validation_input = create_temp_file(
                content="\n".join(all_subdomains_list[:100])
            )

            dns_output = await dnsx_scan(
                domains=str(validation_input), wildcard_check=True
            )
        ...
        if probe_http and all_subdomains_list and config.binary_paths.get("httpx"):
            ...
            httpx_output = await httpx_scan(targets=str(http_targets), tech_detect=True)
        ...
        return format_industrial_result(
            "advanced_subdomain_enumeration", "Error", error=str(e)
        )
...
def nuclei_templates(
    ...
    try:
        if not config.binary_paths.get("nuclei"):
            return json.dumps({"error": "Nuclei is not installed"}, indent=2)
        ...
        raw_data = {"stdout": result["stdout"][:1000], "stderr": result["stderr"][:500]}
        return format_industrial_result(
            "nuclei_templates",
            "Success",
            confidence=1.0,
...
@tool
async def pd_scan_workflow(target: str, workflow: str = "normal") -> str:
...
def notify_alert(
    ...
    try:
        ...
        args = ["-data", str(temp_file), "-webhook", webhook_url]
...
def interactsh_register(
    server: str = "https://interact.sh", correlation_id: str = ""
) -> str:
    """
    ...
    try:
        ...
        if result["success"]:
            match = re.search(
                r"(https?://[a-zA-Z0-9]+\.interact\.sh)", result["stdout"]
            )
            if match:
...
@tool
async def uncover_scan(query: str, engine: str = "shodan", limit: int = 100) -> str:
    ...
    try:
        ...
        env = {}
        if engine == "shodan":
            env["SHODAN_API_KEY"] = config.api_key
        elif engine == "censys":
            env["CENSYS_API_ID"] = config.api_key
...
def get_pd_status() -> str:
    ...
    try:
        ...
        installed = sum(
            1 for tool in tool_info.values() if tool["status"] == "installed"
        )
        total = len(tool_info)
        ...
        status = {
            ...
            "tools": {
                ...
                "installation_rate": f"{(installed / total * 100):.1f}%"
                if total > 0
                else "0%",
                "details": tool_info,
            },
...
def check_windows_compatibility() -> str:
    ...
    try:
        ...
        return format_industrial_result(
            "check_windows_compatibility", "Error", error=str(e)
        )
...
def pd_health_check() -> str:
    ...
    try:
        ...
        installed = sum(
            1 for tool in tools_status.values() if tool["status"] == "installed"
        )
    ...
    try:
        ...
        for port in open_ports:
            ...
            for vuln in vulnerabilities:
                if str(pnum) in str(vuln.get("matched-at", "")) or str(pnum) in str(
                    vuln.get("host", "")
                ):
        ...
        return format_industrial_result(
            "vulnerability_correlator", "Error", error=str(e)
        )


@tool
def import_scan_results(file_path: str, format: str = "auto") -> str:
    """
    ...
    try:
        if not os.path.exists(file_path):
            return format_industrial_result(
                "import_scan_results", "Error", error="File not found"
            )
...
def export_results(
    ...
    try:
        ...
        return json.dumps({"error": f"Export results error: {str(e)}"}, indent=2)


@tool
def generate_results_dashboard(
    scan_results: str, dashboard_type: str = "executive"
) -> str:
    """
    ...
    try:
        ...
        return format_industrial_result(
            "generate_results_dashboard", "Error", error=str(e)
        )
...
async def eminence_discovery_orchestrator(target: str, priority: str = "HIGH") -> str:
    ...
    try:
        ...
        # Priority and Session ID are acknowledged through the context
        # await active_vuln_scanner(target) # Undefined: Placeholder for future integration
