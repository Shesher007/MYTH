...
async def ics_service_mapper(target_ip: str) -> str:
    ...
    try:
        ...
        async def probe_modbus():
            try:
                reader, writer = await asyncio.wait_for(asyncio.open_connection(target_ip, 502), timeout=5)
                ...
                if data and len(data) > 8:
                    return {"service": "Modbus/TCP", "port": 502, "device_info": f"Active Modbus ({len(data)} bytes)", "risk": "CRITICAL"}
            except: pass
            return None
        ...
        async def probe_s7():
            try:
                reader, writer = await asyncio.wait_for(asyncio.open_connection(target_ip, 102), timeout=5)
                ...
                if data and b"\xd0" in data: # COTP Connect Confirm
                    return {"service": "Siemens S7", "port": 102, "device_info": "S7 PLC (COTP Ready)", "risk": "CRITICAL"}
            except: pass
            return None
        ...
        async def probe_bacnet():
            try:
                ...
                sock.close()
                if data:
                    return {"service": "BACnet/IP", "port": 47808, "device_info": "BACnet Device Responding", "risk": "HIGH"}
            except: pass
            return None
        
        modbus_res, s7_res, bacnet_res = await asyncio.gather(probe_modbus(), probe_s7(), probe_bacnet())
        if modbus_res: results.append(modbus_res)
        if s7_res: results.append(s7_res)
        if bacnet_res: results.append(bacnet_res)
...
async def iot_firmware_fingerprinter(target_ip: str) -> str:
    ...
    try:
                ...
                for path in ["/description.xml", "/ssdp/device-desc.xml", "/rootDesc.xml"]:
                    try:
                        ...
                        if resp.status_code == 200 and "<device>" in resp.text.lower():
                            import re
                            manufacturer = re.search(r"<manufacturer>(.*?)</manufacturer>", resp.text, re.I)
                            model = re.search(r"<modelName>(.*?)</modelName>", resp.text, re.I)
                            firmware = re.search(r"<modelNumber>(.*?)</modelNumber>", resp.text, re.I)
                            findings = {
                                "manufacturer": manufacturer.group(1) if manufacturer else "Unknown",
                                "model_name": model.group(1) if model else "Unknown",
                                "firmware_version": firmware.group(1) if firmware else "Unknown",
                                "source_port": port,
                                "source_path": path
                            }
                            break
        ...
        return format_industrial_result("iot_firmware_fingerprinter", "Error", error=str(e))
...
async def autonomous_ics_threat_hunter(target_network: str) -> str:
    ...
    try:
        ...
        async def probe_host(ip):
            ...
            # Quick S7 check
            try:
                reader, writer = await asyncio.wait_for(asyncio.open_connection(ip_str, 102), timeout=2)
                writer.close()
                await writer.wait_closed()
                found.append({"type": "Siemens S7 PLC", "ip": ip_str, "risk": "CRITICAL", "hmi_exposed": False})
            except: pass
            # Quick Modbus check
            try:
                reader, writer = await asyncio.wait_for(asyncio.open_connection(ip_str, 502), timeout=2)
                writer.close()
                await writer.wait_closed()
                found.append({"type": "Modbus/TCP Device", "ip": ip_str, "risk": "HIGH", "hmi_exposed": False})
            except: pass
            return found
        ...
        return format_industrial_result("autonomous_ics_threat_hunter", "Error", error=str(e))
...
async def deep_ics_protocol_analyzer(target_ip: str, protocol: str = "Modbus") -> str:
    ...
    try:
        ...
        if protocol.lower() == "modbus":
            try:
                ...
                if data and len(data) > 8:
                    ...
                    analysis_report["findings"].append({"check": "Read Coils Authorization", "status": "VULNERABLE", "detail": "Read operations allowed without session authentication."})
            except:
        ...
        return format_industrial_result("deep_ics_protocol_analyzer", "Error", error=str(e))

@tool
async def sovereign_iot_remediator(target_ip: str, finding_type: str = "PLC Exposure") -> str:
    """
    ...
    try:
        ...
        plans = {
            "PLC Exposure": [
                {"step": 1, "action": "Protocol Shielding", "detail": "Implement industrial firewalls (e.g., mGuard, Tofino) with Deep Packet Inspection (DPI) for S7/Modbus."},
                {"step": 2, "action": "VLAN Isolation", "detail": "Move PLC to a strictly isolated OT VLAN with no direct Internet/Corp routing."},
                {"step": 3, "action": "Identity Hardening", "detail": "Enable CPU-level passwords and disable unused communication ports (FTP, Web)."}
            ],
            "Unencrypted BACnet": [
                {"step": 1, "action": "Network Tunneling", "detail": "Encapsulate BACnet traffic in an IPsec or WireGuard tunnel between sites."},
                {"step": 2, "action": "Segmented Control", "detail": "Physically separate BMS control network from building Wi-Fi/Guest networks."}
            ],
            "Default ICS Credentials": [
                {"step": 1, "action": "Cryptographic Rotation", "detail": "Force-rotate all controller passwords to minimum 12-char high-entropy strings."},
                {"step": 2, "action": "Console Access", "detail": "Restrict web-based management; use physical console or local-only engineering stations."}
            ]
        }
        
        remediation_plan = plans.get(finding_type, [
            {"step": 1, "action": "General Hardening", "detail": "Apply least-privilege access and enable logging for all OT interactions."}
        ])
        ...
        return format_industrial_result(
            ...
            confidence=1.0,
            impact="MEDIUM",
            raw_data={"target": target_ip, "finding": finding_type, "plan": remediation_plan},
        ...
        return format_industrial_result("sovereign_iot_remediator", "Error", error=str(e))