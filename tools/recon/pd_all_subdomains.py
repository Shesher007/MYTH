...
class IndustryGradeSubdomainFetcher:
    ...
    async def scraper_crtsh(self, domain: str) -> Set[str]:
        ...
        subdomains = set()
        print(f"  [→] Querying crt.sh (Passive)...")
        try:
            url = f"https://crt.sh/?q=%.{domain}&output=json"
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=20) as resp:
                    if resp.status == 200:
                        data = await resp.json(content_type=None) # Handle text/json mismatch
                        for entry in data:
    ...
    async def scraper_hackertarget(self, domain: str) -> Set[str]:
        ...
        subdomains = set()
        print(f"  [→] Querying HackerTarget (Passive)...")
        try:
    ...
    async def scraper_alienvault(self, domain: str) -> Set[str]:
        ...
        subdomains = set()
        print(f"  [→] Querying AlienVault OTX (Passive)...")
        try:
    ...
    async def check_zone_transfer(self, domain: str) -> List[str]:
        ...
        try:
            ...
            nameservers = [line.strip().rstrip('.') for line in out.decode().splitlines() if line.strip()]
            ...
            for ns in nameservers:
                ...
                if shutil.which("dig"):
                    ...
                    p2 = await asyncio.create_subprocess_exec(*axfr_cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
                    out2, _ = await p2.communicate()
                    content = out2.decode()
                    if "Transfer failed" not in content and "failed" not in content.lower() and len(content) > 100:
                        lines = content.splitlines()
                        for l in lines:
                            if domain in l:
                                try:
                                    results.append(l.split()[0].strip('.'))
                                except: pass
            ...
            async with sem:
                try:
                   ...
                   # aliaslist usually contains the CNAME chain.
                    try:
                        res = await loop.run_in_executor(None, socket.gethostbyname_ex, sub)
                        aliases = res[1]
    ...
    async def method_chaos_api(self, domain: str) -> Set[str]:
        ...
        if not api_key: return subdomains
        
        print(f"  [→] Querying Chaos API...")
        try:
    ...
    async def active_verification(self, subdomains: Set[str]) -> Dict[str, Dict[str, Any]]:
                    ...
                    async with aiohttp.ClientSession() as session:
                        try:
                            async with session.get(f"http://{sub}", timeout=5) as resp:
                                live_assets[sub] = {"ip": ip, "status": resp.status, "title": "N/A"}
                        except:
    ...
    async def generate_permutations(self, subdomains: Set[str], domain: str) -> Set[str]:
        """
        ...
        if shutil.which("alterx"):
            ...
            try:
                ...
                for line in out.decode().splitlines():
                    if line.strip(): perms.add(line.strip().lower())
            except: pass

        # Method 2: Advanced Internal Expansion
        prefixes = ["dev", "staging", "prod", "api", "vpn", "internal", "test", "demo", "stage", "lab", "git", "jenkins", "jira"]
    ...
    async def get_all_subdomains_async(self, domain: str, recursive: bool = True) -> Dict[str, Any]:
        """
        ...
        all_subs = set()
        for res in results:
            if isinstance(res, list): all_subs.update(res)
            elif isinstance(res, set): all_subs.update(res)
        
        print(f"[*] Passive Collection: {len(all_subs)} potential subdomains.")
        
        # 5. Cloud Bucket Check (Infinite Addition)
        print(f"[*] Checking for Cloud Bucket Takeovers...")
        takeovers = await self.check_cloud_buckets(all_subs)
        
        # 6. Active Verification
        print(f"[*] Starting Active Verification (DNS + HTTP)...")
        ...
        if recursive and len(live_assets) > 0:
            print(f"[*] Starting Recursive Scan on top 3 deep targets...")
            ...
            for candidate in candidates:
                ...
                rec_subs = set()
                for r in rec_out: rec_subs.update(r)
                if rec_subs:
                    print(f"    [+] Found {len(rec_subs)} nested subdomains for {candidate}!")
