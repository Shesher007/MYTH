...
async def edr_hook_detector() -> str:
    ...
    try:
        ...
        monitored_apis = ["NtWriteVirtualMemory", "NtCreateThreadEx", "NtProtectVirtualMemory", "NtAllocateVirtualMemory"]
        findings = []
        ...
        for api in monitored_apis:
            ...
            kernel32.ReadProcessMemory(kernel32.GetCurrentProcess(), addr, buffer, 1, None)
...
async def sandbox_evader() -> str:
    ...
    try:
        ...
        return format_industrial_result(
            ...
            raw_data={"cores": cores, "ram_gb": round(ram, 1), "disk_gb": round(disk, 1)},
