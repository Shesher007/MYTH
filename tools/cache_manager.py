...
def _refine_category(tool_name: str, current_category: str) -> str:
    ...
    if any(
        k in name_lower
        for k in ["file", "txt", "log", "archive", "compress", "hash", "base64"]
    ):
        return "utilities"
...
def _extract_tool_metadata(
    obj: BaseTool, module_path: str, variable_name: str, default_cat: str
) -> Dict[str, Any]:
            ...
            for name in dir(pkg):
                ...
                if isinstance(obj, BaseTool):
                    if obj.name not in seen_names:
                        ...
                        tools_data.append(
                            _extract_tool_metadata(obj, pkg_name, name, default_cat)
                        )
                            ...
                            if obj.name not in seen_names:
                                ...
                                tools_data.append(
                                    _extract_tool_metadata(
                                        obj, module_name, name, default_cat
                                    )
                                )
