...
from .utilities import format_industrial_result  # noqa: E402
    ...
    def __init__(self, **kwargs):
        ...
        if "input_schema" in kwargs and kwargs["input_schema"]:
            try:
                ...
                logger.error(
                    f"Failed to reconstruct args_schema for {kwargs.get('name')}: {e}"
                )
...
async def get_tools_by_category(
    ...
    source_list = (
        provided_tools if provided_tools is not None else await get_all_tools()
    )
    if not categories:
        return source_list
        ...
        if score > 0:
            ...
            if hasattr(tool, "args_schema") and tool.args_schema:
                try:
                    ...
                    args_schema = {
                        "properties": {},
                        "raw_schema": str(tool.args_schema),
                    }
