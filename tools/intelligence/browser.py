...
class WebAgentManager:
    ...
    async def check_and_install_playwright(self) -> bool:
        ...
        try:
            ...
            if "executable doesn't exist" in error_str or "playwright install" in error_str:
                ...
                notify_system("INFO", "Browser Setup", "First-time setup: Downloading Chromium engine... (This may take a minute)")
                
                print("🚀 [BROWSER] Playwright binaries missing. Launching automated installer...")
                ...
                try:
                    ...
                    if process.returncode == 0:
                        notify_system("SUCCESS", "Browser Setup", "Chromium engine installed successfully.")
                        return True
                    else:
                        notify_system("ERROR", "Browser Setup", f"Automated installation failed: {stderr.decode()}")
                        return False
                except Exception as install_err:
                    notify_system("ERROR", "Browser Setup", f"Installation trigger failed: {install_err}")
                    return False
            return False
    ...
    async def navigate(self, url: str, wait_until: str = "domcontentloaded") -> str:
        ...
        try:
            ...
            self._log_interaction("navigate", {"url": url, "final_url": self.current_url})
    ...
    async def get_page_content(self, include_links: bool = True, include_inputs: bool = True) -> str:
        """
        ...
        try:
            ...
            output_parts.append(content['text'][:1500] + "..." if len(content['text']) > 1500 else content['text'])
            
            if include_links and content['links']:
                output_parts.append("\n## Links")
                for link in content['links'][:15]:
                    output_parts.append(f"  [{link['id']}] {link['text']} → {link['href'][:80]}")
            
            if include_inputs and content['inputs']:
                output_parts.append("\n## Input Fields")
                for inp in content['inputs']:
                    label = inp['label'] or inp['placeholder'] or inp['name'] or inp['type']
                    output_parts.append(f"  [{inp['id']}] {label} (selector: {inp['selector']})")
    ...
    async def click(self, selector: str, wait_for_navigation: bool = False) -> str:
        ...
        target = self.current_frame if hasattr(self, 'current_frame') and self.current_frame else self.page
        ...
        try:
            ...
            if not any(c in selector for c in ['#', '.', '[', '/', ':']):
                ...
                try:
                    element = await target.get_by_text(selector, exact=False).first.element_handle()
                except:
                    try:
                        element = await target.get_by_role("link", name=selector).first.element_handle()
                    except:
                        try:
                            element = await target.get_by_role("button", name=selector).first.element_handle()
                        except:
                            pass
    ...
    async def type_text(self, selector: str, text: str, clear_first: bool = True, press_enter: bool = False) -> str:
        """
        ...
        try:
            ...
            self._log_interaction("type", {"selector": selector, "text": text[:50] + "..." if len(text) > 50 else text})
    ...
    async def screenshot(self, full_page: bool = False) -> str:
        ...
        try:
            ...
            self._log_interaction("screenshot", {"path": str(filepath), "full_page": full_page})
    ...
    async def set_cookie(self, name: str, value: str, domain: str, path: str = "/") -> str:
        """Set a cookie."""
        if not self.is_initialized or not self.context:
            return "ERROR: Browser not initialized."
    ...
    async def download_file(self, url: str = None, click_selector: str = None, save_as: str = None) -> str:
        """
        ...
        try:
            if url:
                        ...
                        if response.status == 200:
                            ...
                            if save_as:
                                ...
                                if 'filename=' in content_disp:
                                    ...
                                    # Extract from URL
                                    filename = url.split('/')[-1].split('?')[0] or f"download_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                            ...
                            self._log_interaction("download", {"url": url, "path": str(filepath), "size": len(content)})
                ...
                if save_as:
                    ...
                    filename = download.suggested_filename or f"download_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                ...
                self._log_interaction("download", {
    ...
    async def download_all_links(self, file_extension: str = None, limit: int = 10) -> str:
        """
        Find and download all files matching a pattern from the current page.
        
        Args:
            file_extension: Filter by extension (e.g., '.pdf', '.zip'). None = all downloadable files.
        ...
        try:
            ...
            links = await self.page.evaluate("""
                (ext) => {
                    const downloadableExtensions = ['.pdf', '.zip', '.rar', '.7z', '.tar', '.gz', 
                                                     '.exe', '.msi', '.dmg', '.deb', '.rpm',
            ...
            if not links:
                return f"❌ No downloadable files found on this page" + (f" matching '{file_extension}'" if file_extension else "")
    ...
    async def handle_next_dialog(self, accept: bool = True, prompt_text: str = None) -> str:
        ...
        try:
            async def dialog_handler(dialog):
                info = f"Dialog message: {dialog.message} | Type: {dialog.type}"
                self._log_interaction("dialog_event", {"message": dialog.message, "type": dialog.type})
    ...
    async def get_element_details(self, selector: str) -> str:
        ...
        try:
            ...
            return f"🔍 Element Details for '{selector}':\n{json.dumps(details, indent=2)}"
        except Exception as e:
    ...
    async def get_local_storage(self, key: str) -> str:
        ...
        try:
            ...
            return f"✅ LocalStorage[{key}]: {value}" if value else f"ℹ️ LocalStorage key '{key}' is empty/null"
        except Exception as e:
    ...
    async def upload_file(self, selector: str, file_path: str) -> str:
        ...
        try:
            ...
            self._log_interaction("upload", {"selector": selector, "file": str(abs_path)})
    ...
    async def drag_and_drop(self, source: str, target: str) -> str:
        ...
        try:
            ...
            ctx = self.current_frame if hasattr(self, 'current_frame') and self.current_frame else self.page
    ...
    async def switch_to_frame(self, selector_or_name: str = None) -> str:
        ...
        try:
            ...
            frame = self.page.frame(name=selector_or_name) or self.page.frame(url=selector_or_name)
    ...
    async def analyze_page_security(self) -> str:
        ...
        try:
            ...
            # 2. Console Analysis
            error_logs = [log for log in self.console_logs if "error" in log.lower() or "exception" in log.lower()]
            auth_logs = [log for log in self.console_logs if any(k in log.lower() for k in ["key", "token", "auth", "secret"])]
            ...
            if analysis['insecure_forms']:
                for f in analysis['insecure_forms']: report.append(f"❌ {f}")
            else:
                report.append("✅ No insecure HTTP forms found.")
                
            report.append("\n### 2. Sensitive DOM Elements")
            if analysis['password_fields']:
                report.append(f"ℹ️ found {len(analysis['password_fields'])} password fields.")
            if analysis['sensitive_inputs']:
                for i in analysis['sensitive_inputs']: report.append(f"⚠️ {i}")
            
            report.append("\n### 3. HTML Comments (Potential Leaks)")
            suspicious_comments = [c for c in analysis['comments'] if any(k in c.lower() for k in ["todo", "fix", "key", "admin", "test"])]
            if suspicious_comments:
                for c in suspicious_comments: report.append(f"⚠️ Comment: <!-- {c} -->")
            else:
            ...
            if error_logs:
                ...
                for l in error_logs[:5]: report.append(f"   {l}")
            if auth_logs:
                report.append(f"⚠️ {len(auth_logs)} Auth-related logs found.")
                for l in auth_logs[:3]: report.append(f"   {l}")
                
            report.append("\n### 5. Client Storage")
            report.append(f"LocalStorage Keys: {', '.join(local_keys) if local_keys else 'None'}")
            report.append(f"SessionStorage Keys: {', '.join(session_keys) if session_keys else 'None'}")
            
            report.append(f"\n### 6. External Scripts ({len(analysis['scripts'])})")
            for s in analysis['scripts'][:5]: report.append(f"• {s}")
            if len(analysis['scripts']) > 5: report.append(f"...and {len(analysis['scripts'])-5} more.")
...
@tool
async def type_into_element(selector: str, text: str, clear_first: bool = True, press_enter: bool = False) -> str:
    """
...
@tool
async def download_file(url: str = None, click_selector: str = None, save_as: str = None) -> str:
    """
...
async def download_all_links(file_extension: str = None, limit: int = 10) -> str:
    ...
    Args:
        file_extension: Filter (e.g., '.pdf', '.csv'). Leave None for everything.
...
@tool
async def handle_next_dialog(accept: Any = True, prompt_text: str = None, **kwargs) -> str:
    """
...
async def browser_forensics_auditor() -> str:
    ...
    if not _browser_manager or not _browser_manager.is_initialized:
        return format_industrial_result("browser_forensics_auditor", "Error", error="Browser not initialized")
    ...
    try:
        ...
        sensitive_keys = [k for k in storage_keys if any(x in k.lower() for x in ["token", "session", "auth", "key"])]
        ...
        return format_industrial_result(
            ...
            confidence=0.9,
            impact="HIGH",
            raw_data={"extensions_count": extensions, "sensitive_storage_keys": sensitive_keys, "history_snapshot": history_count},
        ...
        return format_industrial_result("browser_forensics_auditor", "Error", error=str(e))
...
async def sophisticated_session_cookie_extractor() -> str:
    ...
    global _browser_manager
    if not _browser_manager or not _browser_manager.is_initialized:
        return format_industrial_result("sophisticated_session_cookie_extractor", "Error", error="Browser not initialized")
    ...
    try:
        ...
        session_cookies = [c for c in cookies if any(x in c["name"].lower() for x in ["sess", "auth", "token", "sid"])]
        ...
        return format_industrial_result(
            ...
            confidence=1.0,
            impact="CRITICAL",
            raw_data={"total_cookies": len(cookies), "session_cookies": session_cookies},
        ...
        return format_industrial_result("sophisticated_session_cookie_extractor", "Error", error=str(e))
