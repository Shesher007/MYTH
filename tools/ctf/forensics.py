...
@tool
async def universal_flag_hunter(
    file_path: str, custom_regex: str = r"flag\{[a-zA-Z0-9_\-]+\}"
) -> str:
    """
            ...
            while True:
                ...
                for match in pattern.finditer(chunk):
                    findings.append(
                        {
                            "method": "Plaintext",
                            "flag": match.group().decode(errors="ignore"),
                        }
                    )
                    ...
                    try:
                        ...
                        for submatch in pattern.finditer(decoded):
                            findings.append(
                                {
                                    "method": "Base64 Layer",
                                    "flag": submatch.group().decode(errors="ignore"),
                                }
                            )
                    except Exception:
                        pass
