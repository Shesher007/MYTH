    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as session:
            ...
            async def check_cloud(name, config):
                ...
                try:
                    async with session.get(config['url'], headers=config['headers']) as resp:
                        if resp.status == 200:
            ...
            impact = "HIGH" if detected_provider != "Unknown (On-Prem / Local)" else "LOW"
        ...
        return format_industrial_result("universal_metadata_probe", "Error", error=str(e))
