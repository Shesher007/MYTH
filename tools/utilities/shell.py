...
def _log_audit(command: str, result: Dict[str, Any], cwd: str = None):
    ...
    try:
        ...
        with open(audit_path, "a") as f:
            f.write(f"[{datetime.now().isoformat()}] {command} | result: {result.get('success')} | cwd: {cwd}\n")
    except: pass
...
def _make_command_universal(command: str) -> Tuple[str, List[str]]:
    ...
    if command.strip().lower().startswith(("powershell", "pwsh")):
        if IS_WINDOWS:
            return "powershell", ["-ExecutionPolicy", "Bypass", "-Command"] + [command.replace("powershell", "", 1).replace("pwsh", "", 1).strip()]
        else:
            # PowerShell Core on Unix
            if shutil.which("pwsh"):
                return "pwsh", ["-Command"] + [command.replace("powershell", "", 1).replace("pwsh", "", 1).strip()]
    
    # Detect bash/sh commands on Windows
    if IS_WINDOWS and any(cmd in command.lower() for cmd in ["bash ", "sh ", "curl ", "wget ", "grep ", "awk ", "sed "]):
...
def _execute_with_timeout(command_parts, timeout=100, cwd=None, env=None, capture_output=True):
...
@tool
async def execute_command(command: str, timeout: int = 30, working_directory: str = None) -> str:
    """
    ...
    try:
        ...
        try:
            ...
            process.kill()
            return format_industrial_result("execute_command", "Timeout", error=f"Command timed out after {timeout}s")
        ...
        return format_industrial_result(
            ...
            confidence=1.0,
            impact="MEDIUM",
            raw_data={"stdout": stdout.decode().strip(), "stderr": stderr.decode().strip(), "code": process.returncode},
...
@tool
async def interactive_shell_session(commands: list, session_name: str = "default") -> str:
    """
    ...
    try:
        ...
        cwd = os.getcwd()
        for cmd in commands:
            process = await asyncio.create_subprocess_shell(cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=cwd)
            stdout, stderr = await process.communicate()
            results.append({"cmd": cmd, "stdout": stdout.decode().strip(), "code": process.returncode})
        ...
        return format_industrial_result("interactive_shell_session", "Error", error=str(e))
...
async def execute_powershell(powershell_script: str, timeout: int = 30) -> str:
    ...
    try:
        ...
        process = await asyncio.create_subprocess_exec(
            ps_exec, "-Command", powershell_script,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        stdout, stderr = await asyncio.wait_for(process.communicate(), timeout=timeout)
...
@tool
async def sovereign_process_manager(action: str, target_pid: Optional[int] = None, filter_name: Optional[str] = None) -> str:
    """
    ...
    try:
        if action == "list":
            ...
            for p in psutil.process_iter(['pid', 'name', 'username', 'cpu_percent', 'memory_info']):
                try:
                    ...
                    continue
            return format_industrial_result("sovereign_process_manager", "Success", raw_data={"processes": procs[:50]}, summary=f"Listed {len(procs)} processes (top 50 shown).")

        if not target_pid:
            return format_industrial_result("sovereign_process_manager", "Error", error="Target PID required for this action")
        ...
        try:
            ...
            if action == "terminate":
                p.terminate()
                p.wait(timeout=3)
                return format_industrial_result("sovereign_process_manager", "Terminated", summary=f"Process {target_pid} ({p.name()}) terminated.")
            
            if action == "set_priority":
                # Priority mapping
                levels = {
                    "low": psutil.IDLE_PRIORITY_CLASS if IS_WINDOWS else 19,
                    "normal": psutil.NORMAL_PRIORITY_CLASS if IS_WINDOWS else 0,
                    "high": psutil.HIGH_PRIORITY_CLASS if IS_WINDOWS else -10,
                    "real-time": psutil.REALTIME_PRIORITY_CLASS if IS_WINDOWS else -20
                }
                # logic for setting priority (simplified cross-platform)
                # ...
                return format_industrial_result("sovereign_process_manager", "Priority Updated", summary=f"Priority for {target_pid} updated.")
        except psutil.NoSuchProcess:
            return format_industrial_result("sovereign_process_manager", "Error", error=f"Process {target_pid} not found.")

        return format_industrial_result("sovereign_process_manager", "Error", error="Invalid action")
    except Exception as e:
        return format_industrial_result("sovereign_process_manager", "Error", error=str(e))
...
async def universal_env_validator() -> str:
    ...
    try:
        audit = {
            ...
            "security_hardening": {
                "is_admin": os.getuid() == 0 if not IS_WINDOWS else bool(ctypes.windll.shell32.IsUserAnAdmin()) if 'ctypes' in sys.modules else "Unknown (Check required)",
                "path_integrity": "OK" if len(os.environ.get("PATH", "")) > 0 else "VULNERABLE"
            }
        }
        ...
        return format_industrial_result("universal_env_validator", "Error", error=str(e))

@tool
async def apex_terminal_manager(action: str, session_id: str = "default", command: str = "") -> str:
    """
    ...
    try:
        ...
        if action == "spawn":
            return format_industrial_result("apex_terminal_manager", "Terminal Spawned", raw_data=current_state, summary=f"Stateful session '{session_id}' initialized at {current_state['cwd']}.")
        ...
        if action == "write" and command:
            ...
            if command.strip().startswith("cd "):
                ...
                abs_new_path = os.path.abspath(os.path.join(current_state["cwd"], new_path))
                if os.path.isdir(abs_new_path):
                    current_state["cwd"] = abs_new_path
                    return format_industrial_result("apex_terminal_manager", "CWD Updated", raw_data=current_state, summary=f"Changed directory to {abs_new_path} in session '{session_id}'.")
            ...
            return format_industrial_result(
                ...
                confidence=1.0,
                impact="MEDIUM",
                raw_data={"stdout": stdout.decode().strip(), "stderr": stderr.decode().strip(), "code": process.returncode, "cwd": current_state["cwd"]},
                summary=f"Command '{command}' executed in session '{session_id}' at {current_state['cwd']}."
            )
            
        return format_industrial_result("apex_terminal_manager", "Action Complete", raw_data=current_state, summary=f"Action '{action}' finished for session '{session_id}'.")
    except Exception as e:
...
@tool
async def resonance_cluster_orchestrator(commands: List[str], nodes: List[str] = ["local"]) -> str:
    """
    ...
    try:
        async def execute_on_node(cmd, node):
            ...
            return {"command": cmd, "node": node, "stdout": out.decode().strip(), "code": proc.returncode}
        ...
        return format_industrial_result("resonance_cluster_orchestrator", "Error", error=str(e))

@tool
async def holographic_terminal_persister(session_id: str, action: str = "persist", state_data: Optional[Dict[str, Any]] = None) -> str:
    """
    ...
    try:
        ...
        if action == "persist" and state_data:
            ...
            return format_industrial_result("holographic_terminal_persister", "Persisted", summary=f"Session '{session_id}' holographic state saved.")
            
        if action == "restore":
            if os.path.exists(persist_path):
                with open(persist_path, "r") as f:
                    state = json.load(f)
                return format_industrial_result("holographic_terminal_persister", "Restored", raw_data=state, summary=f"Session '{session_id}' state restored from disk.")
            return format_industrial_result("holographic_terminal_persister", "Not Found", error="No state file exists")
            
        return format_industrial_result("holographic_terminal_persister", "Error", error=f"Invalid action: {action}")
    except Exception as e:
        return format_industrial_result("holographic_terminal_persister", "Error", error=str(e))
