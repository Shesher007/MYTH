...
class SovereignSanitizer:
    ...
    def clean_text(cls, text: str) -> str:
        ...
        clean = clean.replace("<think>", "").replace("</think>", "").replace("think>", "").strip()
        return clean
    ...
    def sanitize_messages(cls, messages: List[BaseMessage]) -> List[BaseMessage]:
        ...
        for msg in messages:
            ...
            if not clean_content and not has_tool_calls and not isinstance(msg, ToolMessage):
                continue

            # Clone with same type but cleaned content, PRESERVING additional_kwargs and specific fields
            if isinstance(msg, HumanMessage):
                new_msgs.append(HumanMessage(content=clean_content, additional_kwargs=msg.additional_kwargs.copy() if msg.additional_kwargs else {}))
                ...
                new_msgs.append(AIMessage(
                    ...
                    tool_calls=getattr(msg, "tool_calls", []).copy() if hasattr(msg, "tool_calls") and msg.tool_calls else [],
                    additional_kwargs=msg.additional_kwargs.copy() if msg.additional_kwargs else {},
                    response_metadata=getattr(msg, "response_metadata", {}).copy() if hasattr(msg, "response_metadata") and msg.response_metadata else {}
                ))
            elif isinstance(msg, SystemMessage):
                new_msgs.append(SystemMessage(content=clean_content, additional_kwargs=msg.additional_kwargs.copy() if msg.additional_kwargs else {}))
