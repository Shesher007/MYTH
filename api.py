...
# This prevents intermittent DNS errors from appearing in the industrial console
...
import threading
import mimetypes
from fastapi import FastAPI, UploadFile, File, WebSocket, WebSocketDisconnect, BackgroundTasks, HTTPException, Body, Form
...
class TerminalFilter(logging.Filter):
    def filter(self, record):
        ...
        noisy_patterns = [
            "/ping", "/health", "/security/threats", "/security/alerts", 
            "/security/network", "/security/sessions", "/security/report",
            "/vpn/status", "/vpn/nodes", "/chat/sessions", "/system/files",
            "THREAT_INTEL", "FIM", "sentinel",
...
class NotificationManager:
    ...
    async def add(self, type: str, title: str, message: str):
        ...
        async with self._lock:
            ...
            if self._queue and self._queue[-1]["title"] == title and self._queue[-1]["message"] == message:
                try:
                    ...
                    if (datetime.now() - last_ts).total_seconds() < 10:
                        ...
                        logger.debug(f"🔄 [NOTIFY] Suppressed duplicate: {title} (Count: {self._queue[-1]['count']})")
...
class SystemTelemetryManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        logger.info(f"📊 [TELEMETRY] Dashboard link established (Active: {len(self.active_connections)})")
        
    async def report_progress(self, task_id: str, progress: int, status: str = "processing"):
    ...
    async def disconnect(self, websocket: WebSocket):
        ...
        logger.info("📊 [TELEMETRY] Dashboard link severed")
    ...
    async def handle_message(self, websocket: WebSocket, data: str):
        ...
        try:
            ...
            if packet.get("type") == "PING":
                # logger.info("📥 [TELEMETRY] Received PING, sending PONG")
            ...
            logger.warning(f"⚠️ [TELEMETRY] Packet error: {repr(e)}")
    ...
    async def broadcast(self, data: dict):
        ...
        results = await asyncio.gather(
            *[connection.send_json(data) for connection in ws_list if connection.client_state == WebSocketState.CONNECTED],
            return_exceptions=True
        )
        
        # Cleanup dead connections identified during broadcast
        disconnected_indices = [i for i, res in enumerate(results) if isinstance(res, Exception)]
            ...
            async with self._lock:
                for ws in to_remove:
                    if ws in self.active_connections:
                        ...
                        logger.debug(f"⚠️ [TELEMETRY] Pruned dead connection during broadcast")
...
async def trigger_telemetry_update():
    ...
    try:
        ...
        packet = {
            ...
            "timestamp": datetime.now().isoformat(),
            "health": health_data if not isinstance(health_data, Exception) else {"status": "error"},
            "sessions": [],
            "files": files_data.get("files", []) if not isinstance(files_data, Exception) else [],
            "security": {
...
CRITICAL_FILES = [
    "api.py", "backend.py", "secrets.yaml", "governance/agent_manifest.yaml", 
    "governance/identity.yaml", "requirements.txt", "pyproject.toml",
    "rag_system/vector_store.py", "rag_system/rag_chain.py",
    "tools/utilities/shell.py", "tools/__init__.py", "mcp_servers/local_servers/system_tools.py"
]
...
class VPNManager:
    def __init__(self):
        ...
        self.nodes = [
            VPNNode(id="nx-01", name="Storage_Nordic", region="Norway", latency="12ms", load=14),
            VPNNode(id="nx-02", name="Neural_Gate_US", region="New York", latency="45ms", load=42),
            VPNNode(id="nx-03", name="Cipher_Swiss", region="Zurich", latency="8ms", load=21),
            VPNNode(id="nx-04", name="Tokyo_Relay", region="Japan", latency="110ms", load=65),
            VPNNode(id="nx-05", name="Berlin_Anchor", region="Germany", latency="15ms", load=38)
        ]
    ...
    def toggle(self, node_id: Optional[str] = None):
        if not self.connected:
            ...
            self.active_node = next((n for n in self.nodes if n.id == node_id), self.nodes[0])
            logger.info(f"🔒 [VPN] Neural Tunnel Established: {self.active_node.name} (TUNNEL_ID: {uuid.uuid4().hex[:8].upper()})")
            # backend.notify_system("SUCCESS", "VPN Tunnel Established", f"Connected via: {self.active_node.name} ({self.active_node.region})")
            pass
        else:
            self.connected = False
            self.active_node = None
            logger.info("🔓 [VPN] Neural Tunnel Terminated.")
...
async def file_integrity_monitor():
    ...
    logger.info(f"🛡️ [FIM] Initialized integrity anchors for {len(CRITICAL_FILES)} core modules")
        ...
        try:
            for f in CRITICAL_FILES:
                ...
                if current != baseline_hashes.get(f) and current != "":
                    ...
                    backend.notify_system("ERROR", "Source Integrity Breach", f"Critical module '{f}' was modified externally.")
                    asyncio.create_task(trigger_telemetry_update())
...
async def honey_pot_watcher():
    ...
    decoys = {
        ...
        "id_rsa.pub.bak": "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABgQCs5Xo... decoy_myth_key\n",
    ...
    logger.info(f"🍯 [HONEYPOT] Anchored {len(decoys)} decoys for intrusion detection.")
        ...
        try:
            ...
            for fpath in current_files:
                if fpath not in honey_baselines:
                    ...
                    security_alerts.append(alert)
                    logger.error(f"🚨 [HONEYPOT] {alert_msg}")
                    ...
                    honey_baselines[fpath] = await asyncio.to_thread(compute_hash, fpath)
                    asyncio.create_task(trigger_telemetry_update())
            ...
            for fpath, old_hash in list(honey_baselines.items()):
                if not os.path.exists(fpath):
                    ...
                    backend.notify_system("WARNING", "Anti-Forensics Detected", alert_msg)
                    ...
                    if fname in decoys:
                        ...
                        # Reset hash
                        honey_baselines[fpath] = await asyncio.to_thread(compute_hash, fpath)
                        logger.warning(f"🛡️ [HONEYPOT] Decoy {fname} reconstructed.")
                    else:
                    ...
                    if current_hash != old_hash:
                        alert_msg = f"Decoy accessed/modified: {os.path.basename(fpath)}"
                        alert = {
                            "id": str(uuid.uuid4())[:8].upper(),
...
async def lifespan(app: FastAPI):
    ...
    """
    logger.info("🚀 [API] Booting Industrial Lifecycle...")
    ...
    logger.info("🛑 [API] Shutting down lifecycle...")
    ...
    try:
        ...
        if hasattr(DocumentProcessor, "_executor"):
            ...
            logger.info("⚙️ [DOC] Document processor executor shutdown.")
    except Exception as e:
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=agent_config.security.allowed_domains if agent_config.security.allowed_domains else ["*"],
...
async def get_dependency_health():
    ...
    if sidecar_dir:
        ...
        for folder in ["nmap", "nodejs"]:
            ...
            if os.path.exists(sub): paths.append(sub)
    ...
    for dep in dependencies:
        found = shutil.which(dep["binary"]) or (os.name == "nt" and shutil.which(f"{dep['binary']}.exe"))
        results.append({
            "name": dep["name"],
            "status": "READY" if found else ("MISSING" if dep["core"] else "OPTIONAL_MISSING"),
            "path": found or "N/A"
        })
...
async def ensure_standalone_assets():
    ...
    """
    from myth_utils.paths import get_app_data_path, get_resource_path
    ...
    if os.path.exists(manifest_src) and not os.path.exists(manifest_dest):
        try:
            ...
            logger.info("📦 [INIT] Synced agent_manifest.yaml to AppData")
        except Exception as e:
            logger.error(f"❌ [INIT] Failed to sync manifest: {e}")
...
async def check_system_dependencies():
    ...
    dependencies = [
        {"name": "Nmap", "binary": "nmap", "link": "https://nmap.org/download.html", "core": True},
        {"name": "Node.js", "binary": "node", "link": "https://nodejs.org/", "core": True},
        {"name": "Docker", "binary": "docker", "link": "https://www.docker.com/products/docker-desktop/", "core": False},
        {"name": "Nuclei", "binary": "nuclei", "link": "https://github.com/projectdiscovery/nuclei", "core": True},
    ]
    ...
    for dep in dependencies:
        ...
        if binary_found:
            found_count += 1
            logger.debug(f"✅ [HEALTH] Found {dep['name']}: {binary_found}")
        else:
            if dep["core"]:
                missing.append(dep)
            else:
                logger.info(f"ℹ️ [HEALTH] Optional dependency missing: {dep['name']}")

    if missing:
        msg = f"Standalone Core Alert: {len(missing)} bundled sidecars are missing or unreachable: " + ", ".join([d["name"] for d in missing])
        await notification_manager.add("WARNING", "Standalone Integrity Check", msg)
        logger.warning(f"⚠️ [HEALTH] Missing core dependencies: {[d['name'] for d in missing]}")
    else:
        logger.info(f"✨ [HEALTH] All core sidecars ({found_count}/{len(dependencies)}) are operational.")
...
async def await_system_ready(timeout: int = 40):
    ...
    try:
        if not backend.REGISTRY["is_ready_event"].is_set():
            logger.info("⏳ [API] Waiting for system readiness event...")
            await asyncio.wait_for(backend.REGISTRY["is_ready_event"].wait(), timeout=timeout)
        return True
    except asyncio.TimeoutError:
        logger.error(f"❌ [API] System readiness timeout after {timeout}s")
        raise HTTPException(status_code=503, detail="System initialization timed out")
    except Exception as e:
        logger.error(f"❌ [API] Readiness check failed: {e}")
...
def should_suppress(content):
    ...
    s = str(content).strip()
    # 1. Exact matches
    if s in ["SIMPLE", "COMPLEX"]: return True
    # 2. JSON-like classification blobs
    if '"classification":' in s or "'classification':" in s: return True
    # 3. Path/Node internal markers
    if s.startswith("AUTO_EFF_VERIFIED_NODE_ID"): return True
    # 4. Filter out raw tags themselves from being displayed
    if s in ["<think>", "</think>"]: return True
    return False
...
class IndustrialLoggingMiddleware:
    def __init__(self, app):
        ...
        self.polling_endpoints = {
            "/ping", "/health", "/chat/sessions", 
            "/security/threats", "/security/alerts", "/security/network", 
            "/security/sessions", "/security/report",
            "/vpn/status", "/vpn/nodes",
            "/notifications/dismiss", "/notifications/clear",
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            logger.info(f"📥 Incoming: {scope['method']} {path}")
            ...
            async def wrapped_send(message):
                if message["type"] == "http.response.start":
                    ...
                    if status_code != 200:
                        logger.info(f"📤 Outgoing: Status {status_code}")
                await send(message)
...
async def authorize_file_change(request: dict):
    ...
    if filename:
        ...
        logger.info(f"🛡️ [FIM] Authorized changes for {filename} (60s window)")
...
async def clear_security_alerts():
    ...
    for f in CRITICAL_FILES:
        ...
        if current_hash: baseline_hashes[f] = current_hash
    logger.warning(f"🧹 [PURGE] Security labs cleared. Re-baselined {len(baseline_hashes)} core modules.")
...
async def mcp_health_sentinel():
    ...
    while True:
        try:
            ...
            target_ports = {cfg["port"]: cfg["category"] for cfg in SSE_CONFIGS.values()}
            if not target_ports:
            ...
            for key, val in new_status.items():
                if val != cached_mcp_status.get(key):
                    if val == "SECURE":
                        ...
                        backend.notify_system("ERROR", "MCP Layer offline", f"Component '{key.upper()}' link lost. Tactical tools unavailable.")
            ...
            logger.warning(f"⚠️ [HEALTH_SENTINEL] Scan error: {e}")
        await asyncio.sleep(10) # Industrial Speed: 10s updates (relaxed from 5s to save CPU)
...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, session_id: str):
        ...
        logger.info(f"🔌 [WS] Connection established for session: {session_id}")
...
def get_avg_latency() -> float:
    if not latency_history: return 0.0
...
async def network_ping_sentinel():
    ...
    logger.info("🌐 [NETWORK_PING] Starting Network Latency Sentinel...")
    ...
    while True:
        try:
            ...
            logger.warning(f"⚠️ [NETWORK_PING] Measurement failed: {e}")
...
async def get_health():
    ...
    try:
        ...
        cpu_usage, memory, disk, uptime_sec, net_io = await asyncio.to_thread(_collect_sys_metrics)
        ...
        base_integrity = 100
        if cpu_usage > 90: base_integrity -= 15
        if vpn_controller.connected: base_integrity += 5
        
        integrity_score = min(100, max(5, base_integrity))

        # Battery Health (Omni-Bundle Dependency Integrity)
        async def _check_batteries():
            sidecars = ["nuclei", "subfinder", "naabu", "httpx", "dnsx", "katana", "subfinder", "shuffledns"]
            binary_integrity = {}
            for s in sidecars:
                ...
                if not found:
                    ...
                    if s_dir:
                        exe_n = f"{s}-x86_64-pc-windows-msvc.exe" if sys.platform == "win32" else s
            ...
            if backend.REGISTRY.get("vector_store"):
                try:
                    stats = await backend.REGISTRY["vector_store"].get_collection_stats("security_docs")
            ...
            return {
                ...
                "browser": "READY" if shutil.which("playwright") or os.environ.get("PLAYWRIGHT_BROWSERS_PATH") else "INSTALLING"
            }
...
async def threat_intel_sentinel():
    ...
    logger.info("🛡️ [THREAT_INTEL] Starting background intelligence sentinel...")
    tick = 0
...
async def get_geo_info(ip: str) -> Dict[str, Any]:
    ...
    if ip in geo_ip_cache: return geo_ip_cache[ip]
...
async def get_network_traffic():
    ...
    try:
        ...
        if connections:
            geo_results = await asyncio.gather(*[get_geo_info(c["remote_ip_raw"]) for c in connections])
...
async def run_port_scan():
    ...
    ports_to_check = [21, 22, 23, 25, 53, 80, 443, 445, 1433, 3306, 3389, 5432, 8000, 8080, 27017]
    open_ports = []
    
    logger.info("🔍 [SCAN] Initiating local node vulnerability assessment...")
...
async def generate_compliance_report():
    ...
    checks.append({"name": "Core Integrity", "status": "FAIL" if tampered else "PASS", "weight": 40})
    if tampered: score -= 40
    
    # 2. RAG Readiness
    rag_ready = bool(backend.REGISTRY.get("vector_store"))
    checks.append({"name": "Knowledge Sovereignty", "status": "PASS" if rag_ready else "INIT", "weight": 20})
    if not rag_ready: score -= 10
    
    # 3. Connection Audit
    ext_conns = [c for c in psutil.net_connections() if c.status == 'ESTABLISHED']
    checks.append({"name": "Network Perimeter", "status": "SECURE" if len(ext_conns) < 50 else "CONGESTED", "weight": 20})
    
    # 4. System Uptime Resilience
    uptime = time.time() - psutil.Process().create_time()
    checks.append({"name": "Runtime Stability", "status": "OPTIMAL" if uptime > 300 else "BOOTSTRAP", "weight": 20})
    ...
    return {
        ...
        "recommendation": "Maintain integrity anchors." if score > 80 else "Immediate forensic audit required."
    }
...
async def get_settings_keys():
    """Retrieve masked API keys from settings."""
    settings = backend.load_settings()
    api_keys = settings.get("api_keys", {})
    
    def mask(s): return f"{s[:4]}...{s[-4:]}" if s and len(s) > 8 else None
    
    return {
        "nvidia_api_key": mask(api_keys.get("NVIDIA_API_KEY") or os.getenv("NVIDIA_API_KEY")),
        "mistral_api_key": mask(api_keys.get("MISTRAL_API_KEY") or os.getenv("MISTRAL_API_KEY"))
    }

@app.post("/settings/keys")
async def update_settings_keys(payload: SettingsKeyRequest):
    """Update API keys and trigger model reload."""
    settings = backend.load_settings()
    ...
    if payload.nvidia_api_key:
        settings["api_keys"]["NVIDIA_API_KEY"] = payload.nvidia_api_key
    if payload.mistral_api_key:
        settings["api_keys"]["MISTRAL_API_KEY"] = payload.mistral_api_key
    ...
    return {"status": "SUCCESS", "message": "Settings updated. Reloading neural matrix..."}
...
async def isolate_node(payload: Dict[str, bool] = Body(...)):
    ...
    logger.warning(f"🛡️ [SECURITY] NODE ISOLATION {'ACTIVATED' if enabled else 'DEACTIVATED'}")
    # Industrial Hook: Surface isolation state
    if enabled:
        backend.notify_system(
            "WARNING", 
            "Global Isolation Matrix", 
            f"Node isolation protocols EXTENDED. Tactical communication RESTRICTED."
        )
...
async def get_architecture():
    ...
    return {
        ...
        "description": "Single Unified Core" if mode == "normal" else "Distributed Warfare Grid"
    }
...
async def set_architecture(request: ArchitectureRequest):
    ...
    if not success:
        raise HTTPException(status_code=400, detail=f"Invalid architecture mode: {request.mode}. Use 'normal' or 'multi'.")
    
    mode = backend.get_architecture_mode()
    logger.info(f"🔄 [API] Architecture switched to: {mode.upper()}")
    return {
        "success": True,
        "mode": mode,
        "display_name": "Solo Strike" if mode == "normal" else "Killchain Matrix"
        ...
        async with session_locks[session_id]:
            ...
            if speaking_mode and backend.REGISTRY.get("vibevoice_processor"):
                try:
                    ...
                    logger.info(f"🎙️ [WS-TTS] Persistent Session Established for {session_id}")
                    
                    # Reset splitter if applicable
                    if hasattr(backend.REGISTRY["vibevoice_processor"].splitter, 'reset'):
                         backend.REGISTRY["vibevoice_processor"].splitter.reset()
                         
                except Exception as e:
                    logger.error(f"❌ [WS-TTS] Failed to start persistent session: {e}")
                    audio_session = None

            audio_tasks = set() # Keep for compatibility, though mostly replaced by consumer

            # audio_consumer: Reads from Gemini and pushes bytes to WebSocket
            async def audio_consumer():
                if not audio_session: return
                try:
                    ...
                    logger.error(f"❌ [WS_AUDIO] Consumer Error: {e}")
            ...
            async def queue_audio_binary(txt):
                ...
                try:
                    ...
                    logger.error(f"❌ [WS_AUDIO] Send Error: {e}")
            ...
            async for event in backend.chatbot.astream_events(
                ...
                if kind == "on_chat_model_start":
                    ...
                    model_name = metadata.get("model_id") or event.get("data", {}).get("model")
                    
                    if not model_name and tags:
                        # Filter out LangGraph internal sequence/graph tags
                        internal_prefixes = ("SEQ:", "ns:", "graph:", "seq:", "langgraph:", "step:")
                        clean_tags = [t for t in tags if not any(t.startswith(p) for p in internal_prefixes)]
                    ...
                    if not model_name or any(model_name.startswith(p) for p in ("SEQ:", "ns:", "seq:", "graph:")):
                        node_model_map = {
                    ...
                    if node_name and node_name != "tools":
                        emoji = "🔄"
                        if "executor" in node_name: emoji = "⚡"
                        elif "router" in node_name: emoji = "🔀"
                        elif "blueprint" in node_name: emoji = "📐"
                        elif "reflection" in node_name: emoji = "🤔"
                    ...
                    input_str = str(inputs)[:200] + "..." if len(str(inputs)) > 200 else str(inputs)
                    ...
                    if "search" in tool_name.lower() or "google" in tool_name.lower():
                        ...
                        msg = f"🔍 Searching for: {str(query)[:100]}"
                    else:
                        msg = f"🛠️ TOOL: `{tool_name}` initialized."
                    
                    await websocket.send_json({"type": "on_tool_execution", "content": msg})
                    # Structured telemetry for UI logic
                    logger.info(f"📤 [WS] Emitting on_tool_start for {tool_name}")
                    await websocket.send_json({"type": "on_tool_start", "name": tool_name})
                    ...
                    if len(out_str) > 300: out_str = out_str[:297] + "..."
                    
                    status_emoji = "✅" if "error" not in out_str.lower() and "exception" not in out_str.lower() else "⚠️"
                    # status_emoji = "✅" if "error" not in out_str.lower() and "exception" not in out_str.lower() else "⚠️"
                    # msg = f"   ↳ {status_emoji} *Output*: `{out_str}`\n"
                    ...
                    if "search" in tool_name.lower() or "google" in tool_name.lower():
                        try:
                            # 1. Regex to find URLs in the output string
                            import re
                            ...
                            if domains:
                                ...
                                icons = ""
                                for d in domains[:5]: # Limit to 5 icons to prevent overflow
                                    icons += f"![{d}](https://www.google.com/s2/favicons?domain={d}&sz=32) "
                                
                                msg = f"📖 Read {count} web pages {icons}"
                                await websocket.send_json({"type": "on_tool_execution", "content": msg})
                        except Exception as e:
                    ...
                    final_output = str(output.content if hasattr(output, 'content') else output)
                    try:
                        if final_output.strip().startswith('{') and final_output.strip().endswith('}'):
                            ...
                            if "status" in parsed and "file" in parsed:
                                f_meta = parsed["file"]
                                final_output = f"🛠️ [OMEGA-PRIME] Industrial Asset: {f_meta.get('name')} | Size: {f_meta.get('size')}"
                    except:
                        pass
                    ...
                    pass # Tool output handled by return value in reasoning stream
                    ...
                    if chunk:
                        ...
                        if reasoning:
                            await websocket.send_json({"type": "on_thought_stream", "content": reasoning})
                        
                        if content:
                            # Stateful tag parsing for <think>
                            if "<think>" in content: in_thinking_state = True; content = content.split("<think>")[-1]
                            if "</think>" in content:
                                thought_part, content = content.split("</think>")
                                if thought_part: await websocket.send_json({"type": "on_thought_stream", "content": thought_part})
                                in_thinking_state = False
                                
                            if in_thinking_state:
                                await websocket.send_json({"type": "on_thought_stream", "content": content})
                            else:
                                # Normal Chat Output
                                    ...
                                    if audio_session:
                                        ...
                                        if len(text_buffer) >= threshold:
                                            ...
                                            if frags:
                                                ...
                                                if idx != -1:
                                                    text_buffer = text_buffer[idx + len(last_frag):]
                                    
                                    await websocket.send_json({"type": "on_chat_model_stream", "content": content})
                    ...
                    output = event.get("data", {}).get("output")
                    if output and hasattr(output, 'content') and not should_suppress(output.content):
                        if node_name == "executor_node":
                            await websocket.send_json({
                                "type": "on_chat_model_end", 
                                "content": SovereignSanitizer.clean_text(str(output.content))
                            })
            ...
            if audio_session:
                ...
                if audio_consumer_task: audio_consumer_task.cancel()
                logger.info(f"🎙️ [WS-TTS] Persistent Session Closed for {session_id}")

    except WebSocketDisconnect:
        logger.info(f"🔌 [WS] Client disconnected abruptly.")
    except Exception as e:
...
async def system_telemetry_endpoint(websocket: WebSocket):
    logger.info("📊 [TELEMETRY] Incoming connection request...")
    """
    ...
    try:
        ...
        logger.warning(f"📊 [TELEMETRY] Socket error: {e}")
        await telemetry_manager.disconnect(websocket)
...
async def system_telemetry_sentinel():
    ...
    logger.info("📊 [TELEMETRY] Initializing Unified System Stream...")
    tick = 0
        ...
        try:
            ...
            if tick % 4 == 0:
                ...
                cached_forensics.update({
                    "network": network_f.get("connections", []) if not isinstance(network_f, Exception) else [],
                    "sessions": sessions_f.get("sessions", []) if not isinstance(sessions_f, Exception) else [],
                    "processes": processes_f.get("processes", []) if not isinstance(processes_f, Exception) else []
                })
            ...
            packet = {
                ...
                "security": {
                    "alerts": alerts_data.get("alerts", []) if not isinstance(alerts_data, Exception) else [],
                    "vpn": vpn_controller.get_status().model_dump()
                }
            }
            ...
            logger.error(f"❌ [TELEMETRY] Sentinel failure: {e}")
            await asyncio.sleep(10)
    ...
    async def event_generator():
        ...
        if request.speaking_mode and backend.REGISTRY.get("vibevoice_processor"):
            try:
                ...
                audio_session = backend.REGISTRY["vibevoice_processor"].create_session()
                await audio_session.__aenter__()
                logger.info(f"🎙️ [TTS] Persistent Session Established for {session_id}")
            except Exception as e:
                logger.error(f"❌ [TTS] Failed to start persistent session: {e}")
                audio_session = None
        ...
        async def audio_consumer():
            ...
            try:
                async for chunk in audio_session.receive():
                    if chunk:
                        # logger.info(f"📤 [AUDIO_PIXEL] Yielding audio bit ({len(chunk)} bytes)")
                ...
                logger.error(f"❌ [AUDIO_PIPE] Consumer Error: {e}")

        audio_consumer_task = asyncio.create_task(audio_consumer()) if audio_session else None
        ...
        try:
            ...
            async def main_stream():
                async for event in backend.chatbot.astream_events(
                    ...
                    if await request.is_disconnected():
                        logger.warning(f"🔌 [SSE] Client disconnected for session {session_id}. Aborting stream.")
                        break
                    ...
                    if kind == "on_chain_start":
                        ...
                        internal_nodes = ["router_node", "reasoning_node", "reflection_node", "blueprint_node", "RunnableSequence", "RunnableLambda"]
                        if name and name not in internal_nodes and not should_suppress(name):
                            yield {
                                "event": "message",
                                "data": json.dumps({
                                    "type": "on_chain_start",
                        ...
                        is_planning = (
                            run_name in ["planning_llm", "reasoning_llm", "blueprint_llm", "router_llm"] or 
                            node_name in ["router_node", "blueprint_node", "reasoning_node"]
                        )
                        ...
                        if content:
                            ...
                            if "<think>" in content: reasoning_trigger = "<think>"
                            elif "think>" in content: reasoning_trigger = "think>"
                            ...
                            if reasoning_trigger:
                                ...
                                if pre and not in_thinking_state:
                                    ...
                                    if not should_suppress(pre):
                                        yield {"event": "message", "data": json.dumps({"type": "on_chat_model_stream", "content": pre, "timestamp": datetime.now().isoformat()})}
                            ...
                            if "</think>" in content and in_thinking_state:
                                ...
                                if thought_chunk:
                                    # Yield final thought
                                    yield {"event": "message", "data": json.dumps({"type": "on_thought_stream", "content": thought_chunk, "timestamp": datetime.now().isoformat()})}
                        ...
                        if reasoning:
                            yield {"event": "message", "data": json.dumps({"type": "on_thought_stream", "content": reasoning, "timestamp": datetime.now().isoformat()})}
                        
                        # Route content based on state
                        if content:
                            if in_thinking_state:
                                # Thoughts always go to thought stream regardless of node
                                yield {"event": "message", "data": json.dumps({"type": "on_thought_stream", "content": content, "timestamp": datetime.now().isoformat()})}
                            else:
                                # Regular chat content
                                # Check for tool calls
                                has_tool_call = hasattr(chunk, 'tool_call_chunks') and bool(chunk.tool_call_chunks)
                                has_accum_tool_call = hasattr(chunk, 'tool_calls') and bool(chunk.tool_calls)
                                ...
                                if not (has_tool_call or has_accum_tool_call):
                                    ...
                                    if is_executor:
                                        ...
                                        if not hasattr(event_generator, "_prefix_buffer"):
                                        ...
                                        if not event_generator._prefix_sent:
                                            ...
                                            if len(event_generator._prefix_buffer) >= 256 or any(term in content for term in ["\n\n", ". ", "? ", "! "]):
                                                clean_prefix = SovereignSanitizer.clean_text(event_generator._prefix_buffer)
                                                if clean_prefix:
                                                    yield {"event": "message", "data": json.dumps({"type": "on_chat_model_stream", "content": clean_prefix, "timestamp": datetime.now().isoformat()})}
                                        ...
                                        if not should_suppress(content):
                                            ...
                                            if audio_session:
                                                ...
                                                # Use splitter from the processor instance
                                                processor = backend.REGISTRY.get("vibevoice_processor")
                                                if processor and hasattr(processor, 'splitter'):
                                                    frags = processor.splitter.split(text_buffer)
                                                    if frags:
                                                        for f in frags:
                                                            if f.strip():
                                                                await audio_session.send(f)
                                            ...
                                            yield {"event": "message", "data": json.dumps({"type": "on_chat_model_stream", "content": content, "timestamp": datetime.now().isoformat()})}
                        ...
                        if is_planning:
                            logger.debug(f"📡 [INTERNAL] Node: {node_name} | Run: {run_name} | Planning: True")
                            if node_name == "router_node":
                                continue
                        else:
                            logger.info(f"📡 [SSE] Kind: {kind} | Node: {node_name} | Run: {run_name}")
                        ...
                        if output and hasattr(output, 'content'):
                            ...
                            if should_suppress(output.content):
                                logger.debug(f"🚫 [FILTER] Suppressed model end content: {str(output.content)[:20]}...")
                                continue

                            # --- SANITIZE FINAL CONTENT ---
                            clean_content = SovereignSanitizer.clean_text(str(output.content))
                            # --- ---------------------- ---

                            # SILENCE PROTOCOL: Don't yield internal planning output at all
                            if is_planning:
                                logger.debug(f"🤫 [SILENCE] Blocked yield for planning node: {node_name}")
                                continue
                            ...
                            logger.info(f"📤 [DEEP_DEBUG] Yielding End: {event_type} | Content: {str(clean_content)[:100]}...")
                        ...
                        if "model" not in kind and "chain" not in kind: # Skip noisy model tokens and chain transitions
                            print(f"👻 [UNHANDLED] Event kind: {kind}, Name: {event.get('name')}")
            ...
            if audio_session:
                await audio_session.__aexit__(None, None, None)
                if audio_consumer_task: audio_consumer_task.cancel()
                logger.info(f"🎙️ [TTS] Persistent Session Closed for {session_id}")

            yield {
                "event": "message",
                "data": json.dumps({"type": "stream_complete", "status": "complete", "session_id": session_id})
            }
...
async def upload_file(
    ...
    collection_name: str = Form("security_docs")
):
    ...
    try:
        ...
        if is_image and backend.REGISTRY.get("image_processor"):
            logger.info(f"🖼️ Analyzing image for security: {file.filename}")
            analysis = await asyncio.to_thread(backend.REGISTRY["image_processor"].security_analysis, file_path)
            
            if not analysis.get("success", True): # defaulting true if key missing to be safe, or check implementation
            ...
            report = analysis.get("analysis_report") or analysis.get("error") or "Analysis failed"
        ...
        if is_audio and backend.REGISTRY.get("audio_processor"):
            logger.info(f"🎙️ Transcribing audio for security: {file.filename}")
            analysis = await backend.REGISTRY["audio_processor"].transcribe_and_analyze(file_path)
            
            if not analysis.get("success", False):
                 raise HTTPException(status_code=422, detail=f"Audio processing failed: {analysis.get('error', 'Unknown error')}")

            # Add analysis to vector store
            if backend.REGISTRY.get("vector_store"):
                doc = Document(
                    page_content=analysis.get("transcription_report", "No transcription available."),
                    metadata={"source": file.filename, "type": "voice_intel", "model": analysis.get("model", "unknown")}
                )
                await backend.REGISTRY["vector_store"].add_langchain_documents(
                    collection_name=collection_name,
                    documents=[doc]
        ...
        if result.get("success"):
            ...
            logger.error(f"❌ [RAG] Processing Error: {error_detail}")
            return {
                "success": False,
                "error": error_detail,
                "filename": file.filename
        ...
        logger.critical(f"❌ [RAG] Critical Exception in upload_file: {e}\n{tb}", exc_info=True)
        # Check backend registry state for debugging
        try:
            reg_status = {k: "Active" for k in backend.REGISTRY.keys()}
            logger.info(f"🔍 [RAG] Backend Registry State: {reg_status}")
        except:
...
async def upload_folder(request: FolderIndexRequest):
    ...
    try:
        logger.info(f"📁 Indexing folder: {request.folder_path}")
        result = await backend.REGISTRY["folder_processor"].aprocess_folder(request.folder_path, request.collection_name)
        return result
    except Exception as e:
...
async def get_folder_summary(request: FolderIndexRequest):
    ...
    try:
        logger.info(f"📁 Analyzing folder for summary: {request.folder_path}")
        summary = backend.REGISTRY["folder_processor"].create_folder_summary(request.folder_path)
...
async def browse_folder():
    ...
    try:
        ...
        logger.info(f"🎬 [BROWSE] Launching worker: {script_path}")
        ...
        if result.returncode == 0 and path:
            logger.info(f"📂 [BROWSE] Folder selected: {path}")
            return {"success": True, "path": path}
        else:
            logger.warning(f"⚠️ [BROWSE] No selection. Exit code: {result.returncode}. Stderr: {err_out}")
            return {"success": False, "error": f"Dialog closed or failed: {err_out}"}
            
    except Exception as e:
        logger.critical(f"❌ [BROWSE] Critical Error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_file_preview(filename: str):
    ...
    is_text = any(mime_type.startswith(t) for t in ["text/", "application/json", "application/javascript", "application/xml", "application/x-sh", "application/x-python", "application/x-perl", "application/x-ruby", "application/x-yaml", "application/x-httpd-php", "application/x-tcl", "application/x-tex"])
    text_extensions = ['py', 'md', 'js', 'jsx', 'ts', 'tsx', 'yaml', 'yml', 'conf', 'log', 'sh', 'sql', 'txt', 'env', 'json', 'xml', 'dockerfile', 'makefile', 'toml', 'ini', 'csv', 'tsv', 'jsonl', 'r', 'php', 'rb', 'java', 'go', 'rs', 'c', 'cc', 'cpp', 'h', 'cs', 'lua', 'pl', 'swift', 'kt', 'dart', 'scala', 'gradle', 'bat', 'ps1', 'cmd', 'bash', 'vbs', 'properties', 'config', 'htaccess', 'tf', 'hcl', 'tfvars', 'sol', 'asm', 's', 'pcre', 'regex', 'rules', 'yara', 'err', 'out', 'stats', 'diag', 'event', 'history']
    if is_text or filename.split('.')[-1].lower() in text_extensions or filename.lower() in ['dockerfile', 'makefile', 'license', 'readme', 'procfile', 'gemfile', 'package.json', 'dockerignore', 'gitignore']:
        try:
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=15.0, verify=False) as client:
            ...
            try:
                ...
                mime_type = head_resp.headers.get("content-type", "application/octet-stream").split(';')[0]
            ...
            async with client.stream("GET", url) as response:
                ...
                is_image_ext = any(url.split('?')[0].lower().endswith(ext) for ext in ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.jfif', '.avif', '.apng', '.pjpeg', '.pjp'])
                result = {
                    ...
                    "hex_dump": None,
                    "is_binary": True,
                    "preview": url if (any(mime_type.startswith(t) for t in ["image/", "audio/", "video/"]) or is_image_ext) else None,
                    "source_url": url
                }
                ...
                is_text = any(mime_type.startswith(t) for t in ["text/", "application/json", "application/javascript", "application/xml", "application/x-sh", "application/x-python", "application/x-perl", "application/x-ruby", "application/x-yaml", "application/x-httpd-php", "application/x-tcl", "application/x-tex"])
                text_extensions = ['py', 'md', 'js', 'jsx', 'ts', 'tsx', 'yaml', 'yml', 'conf', 'log', 'sh', 'sql', 'txt', 'env', 'json', 'xml', 'dockerfile', 'makefile', 'toml', 'ini', 'csv', 'tsv', 'jsonl', 'r', 'php', 'rb', 'java', 'go', 'rs', 'c', 'cc', 'cpp', 'h', 'cs', 'lua', 'pl', 'swift', 'kt', 'dart', 'scala', 'gradle', 'bat', 'ps1', 'cmd', 'bash', 'vbs', 'properties', 'config', 'htaccess', 'tf', 'hcl', 'tfvars', 'sol', 'asm', 's', 'pcre', 'regex', 'rules', 'yara', 'err', 'out', 'stats', 'diag', 'event', 'history']
                if is_text or url.split('?')[0].split('.')[-1].lower() in text_extensions:
                    try:
                        result["content_text"] = chunk[:10000].decode('utf-8', errors='ignore')
                        result["is_binary"] = False
                    except:
                        pass
