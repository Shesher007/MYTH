...
class SystemTelemetryManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        logger.info(
            f"📊 [TELEMETRY] Dashboard link established (Active: {len(self.active_connections)})"
        )
            ...
            async with self._lock:
                for ws in to_remove:
                    if ws in self.active_connections:
                        ...
                        logger.debug(
                            "⚠️ [TELEMETRY] Pruned dead connection during broadcast"
                        )
...
class VPNManager:
    ...
    def toggle(self, node_id: Optional[str] = None):
        if not self.connected:
            ...
            logger.info(
                f"🔒 [VPN] Neural Tunnel Established: {self.active_node.name} (TUNNEL_ID: {uuid.uuid4().hex[:8].upper()})"
            )
...
async def file_integrity_monitor():
    ...
    logger.info(
        f"🛡️ [FIM] Initialized integrity anchors for {len(CRITICAL_FILES)} core modules"
    )
...
async def check_system_dependencies():
    ...
    if missing:
        ...
        logger.warning(
            f"⚠️ [HEALTH] Missing core dependencies: {[d['name'] for d in missing]}"
        )
    else:
        logger.info(
            f"✨ [HEALTH] All core sidecars ({found_count}/{len(dependencies)}) are operational."
        )
...
async def clear_security_alerts():
    ...
    logger.warning(
        f"🧹 [PURGE] Security labs cleared. Re-baselined {len(baseline_hashes)} core modules."
    )
...
def strip_masked_recursive(data):
    ...
    if isinstance(data, dict):
        ...
        cleaned = [
            i for i in (strip_masked_recursive(x) for x in data) if i is not None
        ]
        return cleaned if cleaned else None
    elif isinstance(data, str):
...
async def update_settings_keys(payload: SettingsKeyRequest):
    ...
    if payload.updates:
        ...
        if (
            ...
            and "..." not in payload.nvidia_api_key
            and payload.nvidia_api_key.upper() != "MASKED"
        ):
        ...
        if (
            ...
            and "..." not in payload.mistral_api_key
            and payload.mistral_api_key.upper() != "MASKED"
        ):
...
async def isolate_node(payload: Dict[str, bool] = Body(...)):
    ...
    logger.warning(
        f"🛡️ [SECURITY] NODE ISOLATION {'ACTIVATED' if enabled else 'DEACTIVATED'}"
    )
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        async with session_locks[session_id]:
            ...
            if speaking_mode and backend.REGISTRY.get("vibevoice_processor"):
                try:
                    ...
                    logger.info(
                        f"🎙️ [WS-TTS] Persistent Session Established for {session_id}"
                    )
        ...
        logger.info("🔌 [WS] Client disconnected abruptly.")
    except Exception as e:
        ...
        try:
            ...
            async def main_stream():
                async for event in backend.chatbot.astream_events(
                    ...
                    if await request.is_disconnected():
                        logger.warning(
                            f"🔌 [SSE] Client disconnected for session {session_id}. Aborting stream."
                        )
                        break
                    ...
                    if kind == "on_chain_start":
                        ...
                        if is_planning:
                            logger.debug(
                                f"📡 [INTERNAL] Node: {node_name} | Run: {run_name} | Planning: True"
                            )
                            if node_name == "router_node":
                                continue
                        else:
                            logger.info(
                                f"📡 [SSE] Kind: {kind} | Node: {node_name} | Run: {run_name}"
                            )
                        ...
                        if output and hasattr(output, "content"):
                            ...
                            if should_suppress(output.content):
                                logger.debug(
                                    f"🚫 [FILTER] Suppressed model end content: {str(output.content)[:20]}..."
                                )
                                continue
                            ...
                            if is_planning:
                                logger.debug(
                                    f"🤫 [SILENCE] Blocked yield for planning node: {node_name}"
                                )
                                continue
                            ...
                            logger.info(
                                f"📤 [DEEP_DEBUG] Yielding End: {event_type} | Content: {str(clean_content)[:100]}..."
                            )
                        ...
                        if (
                            ...
                            print(
                                f"👻 [UNHANDLED] Event kind: {kind}, Name: {event.get('name')}"
                            )
...
async def upload_file(
    ...
    try:
        ...
        logger.critical(
            f"❌ [RAG] Critical Exception in upload_file: {e}\n{tb}", exc_info=True
        )
...
async def browse_folder():
    ...
    try:
        ...
        if result.returncode == 0 and path:
            ...
            logger.warning(
                f"⚠️ [BROWSE] No selection. Exit code: {result.returncode}. Stderr: {err_out}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--port", type=int, default=8890, help="Port to bind (default: 8890)"
    )
