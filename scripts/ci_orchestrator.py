#!/usr/bin/env python3
"""
MYTH CI/CD Orchestrator — Platform-Agnostic Build Pipeline
============================================================
...
Stages:
    ...
    test          Run the full MYTH test suite
...
def _banner(stage: str, description: str):
    ...
    print(f"{C.BOLD}{C.CYAN}  ⌬ STAGE: {stage.upper()}{C.RESET}")
...
def _run(
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if check:
                print(
                    f"\n  {C.RED}✘ Command failed (exit {result.returncode}){C.RESET}"
                )
                sys.exit(result.returncode)
        return result
    except FileNotFoundError:
        if not silent:
            print(f"  {C.RED}✘ Command not found: {cmd[0]}{C.RESET}")
        if check:
            sys.exit(1)
        return None
...
def stage_lint_python(*, dry_run=False, verbose=False, **_):
    ...
    if result is None:
        print(f"  {C.YELLOW}⚠ Tool 'ruff' not found. Skipping.{C.RESET}")
        return True

    if result.returncode != 0:
        print(f"  {C.YELLOW}⚠ Python linting found issues{C.RESET}")
    else:
        print(f"  {C.GREEN}✔ Python linting passed{C.RESET}")
        # Run formatter too
        _run(
            ["uv", "run", "ruff", "format", "."],
            dry_run=dry_run,
...
def stage_lint_js(*, dry_run=False, verbose=False, **_):
    ...
    if success:
        print(f"  {C.GREEN}✔ JavaScript linting passed{C.RESET}")
    else:
        print(f"  {C.YELLOW}⚠ JavaScript linting found issues{C.RESET}")
...
def stage_lint_rust(*, dry_run=False, verbose=False, **_):
    ...
    if result is not None and result.returncode == 0:
        print(f"  {C.GREEN}✔ Rust linting passed{C.RESET}")
        return True

    print(f"  {C.YELLOW}⚠ Rust linting found issues{C.RESET}")
    return True  # Advisory


def stage_test(*, dry_run=False, verbose=False, fast=True, **_):
    """Run the MYTH test suite."""
...
def stage_build_backend(
    *, dry_run=False, verbose=False, skip_sync=False, target=None, **_
    ...
    if secrets_bundle:
        env_extra["MYTH_SECRETS_BUNDLE"] = secrets_bundle
        print(f"  {C.DIM}🔒 Secrets bundle detected in environment{C.RESET}")
    ...
    print(f"\n  {C.GREEN}✔ Backend packaged for {target}{C.RESET}")
    return True
...
def stage_build_desktop(*, dry_run=False, verbose=False, target=None, **_):
    ...
    print(f"\n  {C.GREEN}✔ Desktop build complete for {target}{C.RESET}")
    return True
...
def stage_build_docker(*, dry_run=False, verbose=False, **_):
    ...
    print(f"\n  {C.GREEN}✔ Docker image built: {image_name}{C.RESET}")
    return True
...
def stage_deploy_website(*, dry_run=False, verbose=False, **_):
    ...
    try:
        ...
        site_name = f"{id_sect.get('name', 'myth').lower()}-website"
    except Exception:
        pass

    if not os.path.isdir(WEBSITE_DIR):
        print(f"  {C.YELLOW}⚠ Website directory not found, skipping{C.RESET}")
        return True
    ...
    # Deploy (Cloudflare Pages — override with env var if needed)
    ...
    print(f"\n  {C.GREEN}✔ Website deployed to {site_name}{C.RESET}")
    return True
...
def stage_check_env(*, dry_run=False, verbose=False, **_):
    ...
    for bin_name, name, is_critical in tools:
        ...
        if not found_path:
            if is_critical:
                ...
                print(f"  {C.RED}✘ CRITICAL: Missing tool: {name}{C.RESET}")
            else:
                missing_optional.append(name)
                print(f"  {C.DIM}⚠ OPTIONAL: Missing tool: {name}{C.RESET}")
        else:
    ...
    if missing_critical:
        print(
            f"\n  {C.RED}✘ Mission-critical tools missing. Pipeline cannot proceed.{C.RESET}"
        )
        sys.exit(1)
    ...
    print(
        f"\n  {C.GREEN}✔ Phase 1 Environment readiness check passed (Critical tools verified){C.RESET}"
    )
    return True
...
def stage_check_secrets(*, dry_run=False, **_):
    ...
    for s in required_secrets:
        if s not in os.environ:
            missing.append(s)
            print(f"  {C.YELLOW}⚠ Missing secret: {s}{C.RESET}")
        else:
            print(f"  {C.GREEN}✔ Secret found: {s}{C.RESET}")

    if missing:
        print(
            f"\n  {C.YELLOW}⚠ Warning: Some deployments/stages will fail without these secrets.{C.RESET}"
        )
    else:
        print(f"\n  {C.GREEN}✔ All core secrets are present{C.RESET}")
    return True
...
def stage_linux_packages(*, dry_run=False, verbose=False, **_):
    ...
    print(f"\n  {C.GREEN}✔ Linux packages generated{C.RESET}")
    return True
...
STAGES = {
    ...
    "test": (stage_test, "Run MYTH test suite"),
...
def main():
    parser = argparse.ArgumentParser(
        description="MYTH CI/CD Orchestrator — Platform-Agnostic Build Pipeline",
...
{C.BOLD}{C.CYAN}
 ┌─────────────────────────────────────────────────┐
 │   ⌬  MYTH CI/CD ORCHESTRATOR                    │
 │   Platform-Agnostic Build Pipeline               │
 └─────────────────────────────────────────────────┘{C.RESET}
""")
    ...
    for stage_raw in args.stages:
        ...
        for name in stages_to_run:
            ...
            try:
                ...
                results[name] = False
                if e.code != 0:
                    print(f"\n{C.RED}✘ Pipeline failed at stage: {name}{C.RESET}")
...
def _print_summary(results: dict, elapsed: float):
    ...
    if all_passed:
        ...
        print(f"\n  {C.RED}{C.BOLD}✘ Pipeline failed{C.RESET}")
    print()
