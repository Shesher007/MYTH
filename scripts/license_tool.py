...
try:
    ...
    print("❌ Error: 'cryptography' library not found. Please run: uv pip install cryptography")
    exit(1)
...
def create_certificate(priv_key_hex, activation_key, device_fp, tier="Pro", days=365):
    ...
    expiration = (datetime.utcnow() + timedelta(days=days)).strftime("%Y-%m-%d") if days > 0 else "perpetual"
