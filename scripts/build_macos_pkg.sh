...
PKG_OUTPUT="dist/MYTH-1.1.3.pkg"
...
PKG_ARGS=(--root "$APP_PATH" --identifier "com.shesher010.myth" --version "1.1.3" --install-location "/Applications/MYTH.app")
...
if [ -n "$SIGNING_ID" ]; then
    ...
    productsign --sign "$SIGNING_ID" "$PKG_OUTPUT" "dist/MYTH-1.1.3-signed.pkg"
    mv "dist/MYTH-1.1.3-signed.pkg" "$PKG_OUTPUT"
fi
