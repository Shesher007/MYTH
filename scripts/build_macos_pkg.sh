...
PKG_OUTPUT="dist/MYTH-1.1.6.pkg"
...
PKG_ARGS=(--root "$APP_PATH" --identifier "com.shesher007.myth" --version "1.1.6" --install-location "/Applications/MYTH.app")
...
if [ -n "$SIGNING_ID" ]; then
    ...
    productsign --sign "$SIGNING_ID" "$PKG_OUTPUT" "dist/MYTH-1.1.6-signed.pkg"
    mv "dist/MYTH-1.1.6-signed.pkg" "$PKG_OUTPUT"
fi
