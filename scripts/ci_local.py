#!/usr/bin/env python3
"""
MYTH Local CI Runner — Developer Pre-Push Gate
================================================
...
Environment Variables:
    MYTH_CI_VERBOSE=1    Show full command output
    MYTH_CI_TARGET=...   Override target triple
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="MYTH Local CI Runner — Developer Pre-Push Gate",
    ...
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Show full command output (or set MYTH_CI_VERBOSE=1)",
    )
    parser.add_argument(
        "--target",
        default=None,
        help="Override Rust target triple (or set MYTH_CI_TARGET)",
    )
    ...
    # Env overrides
    if os.environ.get("MYTH_CI_VERBOSE") == "1":
        args.verbose = True
    if os.environ.get("MYTH_CI_TARGET"):
        args.target = args.target or os.environ["MYTH_CI_TARGET"]

    # Header
    print(f"""
{BOLD}{CYAN}
 ┌─────────────────────────────────────────────────┐
 │   ⌬  MYTH LOCAL CI RUNNER                       │
