...
class CircuitBreaker:
    ...
    def __init__(self, provider: str, failure_threshold: int = 5, recovery_timeout: float = 30.0):
    ...
    def state(self) -> str:
        ...
        if self._state["state"] == self.OPEN:
            if time.time() - self._state["last_failure"] > self.recovery_timeout:
                ...
                logger.info(f"🔄 [CIRCUIT] {self.provider}: OPEN → HALF_OPEN (testing recovery)")
    ...
    def record_failure(self, error: Exception):
        ...
        if self._state["failures"] >= self.failure_threshold:
            ...
            logger.warning(f"🔴 [CIRCUIT] {self.provider}: CLOSED → OPEN (threshold reached: {self.failure_threshold})")
...
class ReliableLLM(BaseChatModel):
    ...
    def _init_model(self, force_rotate: bool = False):
        ...
        if not new_key:
            logger.critical(f"❌ [RELIABLE_LLM] No valid keys available for {self.provider}!")
        ...
        try:
            ...
            logger.debug(f"✅ [RELIABLE_LLM] Model initialized: {self.provider}/{self.model_name}")
    ...
    def _classify_error(self, error: Exception) -> str:
        ...
        if any(x in err_str for x in ["401", "unauthorized", "auth", "forbidden", "403"]):
            return "AUTH"
        ...
        **kwargs
        ...
        for attempt in range(self.max_retries):
            try:
                if tools:
                    serialized_tools = self._prepare_tools(tools)
                    bound_model = self._underlying_model.bind_tools(serialized_tools, **kwargs)
                    response = bound_model.invoke(messages, stop=stop)
                else:
                    response = self._underlying_model.invoke(messages, stop=stop, **kwargs)
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if tools:
                    ...
                    response = await self._underlying_model.ainvoke(messages, stop=stop, **kwargs)
                ...
                if error_type == "CONTEXT":
                    logger.error(f"❌ [RELIABLE_LLM] Context error (non-retryable): {e}")
                ...
                if error_type == "AUTH":
                    ...
                    try:
                        ...
                        logger.info(f"🔄 [RELIABLE_LLM] Key rotated for {self.provider}")
                    except ValueError:
                        logger.critical(f"❌ [RELIABLE_LLM] All keys exhausted for {self.provider}!")
                        self._circuit.record_failure(e)
                        raise
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        if hasattr(self._underlying_model, '_stream'):
            yield from self._underlying_model._stream(messages, stop=stop, run_manager=run_manager, **kwargs)
        else:
