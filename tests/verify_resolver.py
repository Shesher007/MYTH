...
def test_resolver():
    ...
    try:
        ...
        if "pc-windows-msvc" in forced_resolved:
            print("   ✅ SUCCESS: Correctly resolved to bundled binary when missing from PATH.")
        else:
            print(f"   ❌ FAILED: Did not resolve to bundled binary. Got: {forced_resolved}")
