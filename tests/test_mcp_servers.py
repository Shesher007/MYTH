...
MCP_SERVERS_DIR = os.path.abspath(
...
def _check_server_exports(mod) -> str:
    ...
    for name in dir(mod):
        ...
        if obj_type in ("FastMCP", "Server", "Starlette") or name in (
            "mcp",
            "app",
            "server",
        ):
    ...
    for group_name, servers in MCP_SERVERS.items():
        ...
        for mod_path, description in servers.items():
            ...
            if mod:
                ...
                tracker.record(
                    f"{description} ({mod_path}) [{export_info}]", Status.PASS, elapsed
                )
            else:
                tracker.record(
                    f"{description} ({mod_path})", Status.FAIL, elapsed, error=err
                )
