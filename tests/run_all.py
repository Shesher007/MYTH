...
from conftest import ResultTracker, C
...
def main():
    ...
    parser.add_argument("--fast", action="store_true",
                        help="Skip slow tool invocation tests")
...
""")
    ...
    if args.fast:
        modules_to_run = [(m, d) for m, d in modules_to_run if m != "test_tool_invocations"]
    ...
    for module_name, description in modules_to_run:
        ...
        try:
            ...
            test_module = __import__(f"testing.{module_name}", fromlist=["run"])
            test_module.run(tracker)
        except Exception as e:
            import traceback
            tracker.begin_module(module_name)
            tracker.record(f"MODULE CRASH: {module_name}", 
                          __import__("testing.conftest", fromlist=["Status"]).Status.FAIL,
                          0, error=traceback.format_exc())
            tracker.end_module()
