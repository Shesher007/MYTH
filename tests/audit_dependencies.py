...
try:
    ...
    for d in distributions():
        ...
        INSTALLED_PACKAGES[name] = {"version": d.version, "is_in_venv": is_in_venv}
    ...
    try:
        ...
        try:
            ...
            for match in re.finditer(
                r"^\s*from\s+([A-Za-z][\w.]+)\s+import", source, re.MULTILINE
            ):
                imports.add(match.group(1).split(".")[0])
        except Exception:
            pass
...
def extract_js_imports(filepath: Path):
    ...
    try:
        ...
        for match in re.finditer(
            ...
            if pkg.startswith("@"):
                pkg = (
                    match.group(1).split("/")[0] + "/" + match.group(1).split("/")[1]
                    if "/" in match.group(1)
                    else pkg
                )
            imports.add(pkg)
...
def extract_system_binaries(filepath: Path):
    ...
    try:
        ...
        for match in re.finditer(
            r"""['"](?:which|command)\s*['"]\s*,\s*['"](\w+)['"]""", source
        ):
            binaries.add(match.group(1))
...
def extract_external_apis(filepath: Path):
    ...
    try:
        ...
        for match in re.finditer(
            r"""https?://(?:api\.|www\.)?([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})""", source
        ):
...
def main():
    ...
    report.append(
        f"> Scanned: **{py_files_scanned}** Python files, **{js_files_scanned}** JS/JSX files"
    )
    report.append("")
    ...
    report.append("")
    report.append(
        f"**{len(third_party_imports)}** unique third-party packages imported across {py_files_scanned} Python files."
    )
    report.append("")
    ...
    sorted_tp = sorted(
        third_party_imports.items(), key=lambda x: len(x[1]), reverse=True
    )
    install_pkgs = []
    ...
    for i, (mod, files) in enumerate(sorted_tp, 1):
        ...
        if not is_installed and mod in [
            "win32",
            "win32api",
            "win32con",
            "win32file",
        ]:  # special handling for pywin32 modules often missed
        ...
        report.append(
            f"| {i} | `{mod}` | {status_icon} {installed_version}{loc_suffix} | {len(files)} | {sample} |"
        )
    report.append("")
    ...
    report.append("")
    sorted_stdlib = sorted(
        stdlib_imports.items(), key=lambda x: len(x[1]), reverse=True
    )
    ...
    report.append("")
    for mod, files in sorted(
        internal_imports.items(), key=lambda x: len(x[1]), reverse=True
    ):
    ...
    report.append("")
    report.append(
        f"**{len(all_js_imports)}** unique packages imported across {js_files_scanned} JS/JSX files."
    )
    report.append("")
    ...
    if all_binaries:
        report.append(
            f"**{len(all_binaries)}** external programs/tools invoked via subprocess or os.system."
        )
        report.append("")
