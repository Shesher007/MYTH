...
def run(tracker: ResultTracker = None):
    ...
    for mod_path, checks in RAG_MODULES.items():
        ...
        if "class" in checks:
            ...
            if hasattr(mod, cls_name):
                ...
                if isinstance(cls, type):
                    tracker.record(f"  {mod_path}.{cls_name} (class exists)", Status.PASS, 0)
                else:
                    tracker.record(f"  {mod_path}.{cls_name} is not a class", Status.WARN, 0)
            else:
        ...
        if "callable" in checks:
            ...
            if hasattr(mod, fn_name):
                ...
                if callable(fn):
                    tracker.record(f"  {mod_path}.{fn_name} (callable exists)", Status.PASS, 0)
                else:
                    tracker.record(f"  {mod_path}.{fn_name} is not callable", Status.WARN, 0)
            else:
                # Fallback: check for any exported callable
                exports = [n for n in dir(mod) if not n.startswith('_') and callable(getattr(mod, n, None))]
                if exports:
                    tracker.record(f"  {mod_path} has {len(exports)} callable(s) (expected '{fn_name}' not found)",
                                  Status.WARN, 0)
                else:
    ...
    if mod:
        ...
        tracker.record(f"rag_system.__init__ ({len(all_exports)} exports)", Status.PASS, elapsed)
    else:
