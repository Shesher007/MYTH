...
class DocumentProcessor:
    ...
    def __init__(self, 
        ...
        if not hasattr(DocumentProcessor, "_executor"):
            DocumentProcessor._executor = ThreadPoolExecutor(max_workers=os.cpu_count() or 4)
        ...
        self.supported_formats = {
            # Text formats
            '.txt', '.md', '.rst', '.json', '.xml', '.csv', '.yaml', '.yml',
            # Document formats
    ...
    def get_file_hash(self, file_path: str) -> str:
        ...
        with open(file_path, "rb") as f:
            for byte_block in iter(lambda: f.read(65536), b""): # Larger block for speed
                sha256_hash.update(byte_block)
        return sha256_hash.hexdigest()
    
    async def process_batch(self, file_paths: List[str], task_id: str = "batch_index") -> List[LangchainDocument]:
                ...
                try:
                    ...
                    if backend.REGISTRY.get("progress_callback"):
                        ...
                        if asyncio.iscoroutinefunction(cb):
                            await cb(task_id, progress, f"processed {processed_count}/{total_files}: {os.path.basename(path)}")
                        else:
                            cb(task_id, progress, f"processed {processed_count}/{total_files}: {os.path.basename(path)}")
    ...
    def extract_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        if path.suffix.lower() in ['.jpg', '.jpeg', '.png', '.tiff', '.tif', '.jfif', '.webp']:
    ...
    async def process_document(self, file_path: str, current_depth: int = 0) -> List[LangchainDocument]:
        ...
        if file_hash in self.processed_cache:
            logger.info(f"⏩ [SKIP] Already processed in this session: {Path(file_path).name}")
            return []
        ...
        try:
            ...
            if extension in ['.pdf', '.docx', '.pptx', '.xlsx', '.doc', '.ppt']:
            ...
            # 2. Extract Multimodal Content (Vision/Audio) with Semaphore Protect
            elif extension in {'.jpg', '.jpeg', '.png', '.webp', '.bmp', '.tiff', '.tif', '.jfif'}:
                async with self.api_semaphore:
                    docs = await self._process_image(file_path, metadata)
                for d in docs: d.metadata["source_type"] = "vision_analysis"
                documents.extend(docs)
                
            elif extension in {'.mp3', '.wav', '.flac', '.m4a', '.ogg'}:
                async with self.api_semaphore:
                    docs = await self._process_audio(file_path, metadata)
                for d in docs: d.metadata["source_type"] = "audio_transcript"
                documents.extend(docs)
                ...
                docs = self._process_security_xml(file_path, metadata)
                            ...
                            '.py', '.js', '.java', '.cpp', '.c', '.html', '.css', '.sh', '.sql']:
                docs = self._process_with_unstructured(file_path, metadata)
            else:
            ...
            for doc in docs:
                if len(doc.page_content) > 8000:
                    logger.info(f"⚡ [DOC] Oversized chunk detected ({len(doc.page_content)} chars). Applying secondary splitting.")
                    sub_chunks = self.text_splitter.split_text(doc.page_content)
                    for i, sub_chunk in enumerate(sub_chunks):
                        new_metadata = doc.metadata.copy()
                        new_metadata["chunk_id"] = f"{new_metadata.get('chunk_id', 0)}_sub_{i}"
                        final_docs.append(LangchainDocument(page_content=sub_chunk, metadata=new_metadata))
                else:
                    final_docs.append(doc)
            
            # 4. Final Security Pass (Secret Discovery)
            for doc in final_docs:
                doc.metadata = self.secret_scanner.flag_document(doc.page_content, doc.metadata)
    ...
    def _process_image(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
        ...
        try:
            if self.nvidia_api_key:
                ...
                logger.info(f"🔬 Conducting deep security audit of {metadata['file_name']} via NVIDIA Vision...")
            ...
            if "exif" in metadata:
                exif_text = "\n".join([f"{k}: {v}" for k, v in metadata["exif"].items()])
                doc_metadata = metadata.copy()
                doc_metadata.update({
                    "content_type": "exif_metadata",
                    "source_type": "image_exif"
                })
                documents.append(LangchainDocument(
                    page_content=exif_text,
                    metadata=doc_metadata
                ))
    ...
    async def _process_archive(self, file_path: str, metadata: Dict, current_depth: int = 0) -> List[LangchainDocument]:
        """Process archive files recursively (Async)"""
        documents = []
        try:
            logger.info(f"📦 Extracting archive {metadata['file_name']} for ingestion (Depth: {current_depth})...")
            result = self.archive_extractor.extract_archive(file_path, current_depth=current_depth)
    ...
    async def _process_audio(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
        ...
        try:
            ...
            logger.info(f"🎙️  Transcribing security audio {metadata['file_name']} via Mistral Voxtral...")
    ...
    def _process_with_unstructured(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
        """Process document using unstructured with high-res strategies and semantic chunking"""
        try:
            if not UNSTRUCTURED_AVAILABLE:
                logger.warning("Unstructured library not available, falling back to text processing")
    ...
    def _process_spreadsheet(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
    ...
    def _process_pcap(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                ...
                if ips:
                    intel.append(f"Detected IP Addresses: {', '.join([ip.decode() for ip in ips])}")
                ...
                security_keywords = {b"HTTP", b"GET", b"POST", b"DNS", b"SSH", b"FTP", b"TLS", b"SSL"}
                found_keywords = [s.decode() for s in strings if s in security_keywords]
                if found_keywords:
                    intel.append(f"Detected Protocols/Methods: {', '.join(found_keywords)}")
    ...
    def _process_as_text(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
    ...
    def _process_security_json(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                doc_metadata = metadata.copy()
                doc_metadata.update({"chunk_id": i, "total_chunks": len(chunks), "source_type": "security_json"})
    ...
    def _process_security_xml(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                doc_metadata = metadata.copy()
                doc_metadata.update({"chunk_id": i, "total_chunks": len(chunks), "source_type": "security_xml"})
                documents.append(LangchainDocument(page_content=chunk, metadata=doc_metadata))
            return documents
        except Exception as e:
            return self._process_with_unstructured(file_path, metadata)

    def _process_pdf_pypdf(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
        """Robust PDF extraction using pypdf"""
        try:
