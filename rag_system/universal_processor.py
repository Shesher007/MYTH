...
class UniversalFileProcessor:
    ...
    def detect_file_type(self, file_path: str) -> Dict[str, Any]:
        ...
        entropy = self._calculate_entropy_full_file(
            file_path
        )  # Recalculate for full file
        ...
        metadata = {
            ...
            or header_info.get("magic", "Unknown Binary"),
        }
        ...
        if entropy > 7.5:
            ...
            metadata["entropy_rating"] = (
                "Suspiciously High (Likely Encrypted or Packed)"
            )
            is_suspicious = True
        elif entropy > 6.5:
        ...
        if extension in {".exe", ".bin", ".elf", ".dll", ".so"}:
            ...
            if "text/" in mime_type or "json" in mime_type or "xml" in mime_type:
                ...
                or "application/vnd.openxmlformats" in mime_type
            ):
                category = "Document"
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                if header_bytes:
                    ...
                    header_info["hex_preview"] = binascii.hexlify(
                        header_bytes[:32]
                    ).decode("utf-8")

                    # ASCII representation
                    ascii_preview = "".join(
                        chr(b) if 32 <= b < 127 else "." for b in header_bytes[:64]
                    )
    ...
    def _categorize_file(
        self, extension: str, mime_type: str, file_description: str
    ) -> str:
        ...
        if any(
            x in mime_type
            for x in ["text/", "application/json", "application/xml", "application/pdf"]
        ):
            return "text"
    ...
    def process_any_file(
        ...
        if category == "text":
            ...
            logger.info(
                f"Universal Processor: Handling fallback for text file {file_path}"
            )
            try:
                # Try UTF-8 first
                ...
                if (
                    not content
                    or self.mime_detector
                    and "compressed"
                    in self.mime_detector.from_buffer(content.encode("utf-8")[:1024])
                ):
                    ...
                    documents.append(
                        LangchainDocument(page_content=content, metadata=doc_metadata)
                    )
            except Exception as e:
            ...
            logger.info(
                f"Universal Processor: Handling fallback for image file {file_path}"
            )
...
"""
        ...
        documents.append(
            LangchainDocument(page_content=info_content, metadata=doc_metadata)
        )
...
"""
        ...
        return [LangchainDocument(page_content=content, metadata=doc_metadata)]
...
"""
    ...
    def _process_media(self, file_path: str, metadata: Dict) -> List[LangchainDocument]:
...
"""
        ...
        if media_info:
            ...
            if "format" in media_info:
                content += (
                    f"Format: {media_info['format'].get('format_name', 'Unknown')}\n"
                )
                content += (
                    f"Duration: {media_info['format'].get('duration', 'Unknown')}s\n"
                )
                content += (
                    f"Bitrate: {media_info['format'].get('bit_rate', 'Unknown')} bps\n"
                )
...
"""
    ...
    def _to_langchain_doc(self, res: Dict[str, Any]) -> LangchainDocument:
        ...
        return LangchainDocument(
            page_content=res.get("content", ""), metadata=res.get("metadata", {})
        )
