...
class ImageProcessor:
    ...
    def __init__(self, nvidia_api_key: Optional[str] = None, vision_model: Optional[str] = None):
        ...
        logger.info(f"🎨 [VISION] ImageProcessor initialized with node: {self.vision_model_id}")
    ...
    def detect_objects(self, image_path: str, 
                      target_objects: Optional[List[str]] = None) -> Dict[str, Any]:
        """Detect objects in image using Vision API"""
        if not target_objects:
            target_objects = ["servers", "network gear", "computers", "people", "security cameras", "ID badges", "sensitive documents"]
        ...
        return {
            "detected_objects_summary": findings,
            "image_path": image_path
        }
    ...
    def analyze_image_similarity(self, image1_path: str, 
                                image2_path: str) -> Dict[str, Any]:
