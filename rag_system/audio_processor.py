...
class AudioProcessor:
    ...
    async def transcribe_and_analyze(self, audio_path: str, prompt: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"📤 [AUDIO_API] Sending direct multimodal request to Mistral...")
            ...
            for attempt in range(max_retries + 1):
                ...
                if response.status_code == 429:
                    if attempt < max_retries:
                        ...
                        logger.warning(f"⚠️ [AUDIO_API] Rate limit hit (429). Retrying in {delay}s... (Attempt {attempt + 1}/{max_retries})")
                        import asyncio
                        await asyncio.sleep(delay)
                        continue
                    else:
                        error_msg = response.text
                        logger.error(f"❌ [AUDIO_API] Max retries exceeded for rate limit.")
                        return {"success": False, "error": f"API Rate Limit Exceeded after {max_retries} retries. Please check your Mistral quota."}
                ...
                if response.status_code != 200:
                    ...
                    logger.error(f"Mistral Audio API reported error {response.status_code}: {error_msg} | Size: {file_size}b")
                    return {"success": False, "error": f"Mistral API Error {response.status_code}: {error_msg}"}
            ...
            if "choices" not in data or not data["choices"]:
                return {"success": False, "error": "Invalid API response: No choices returned"}
