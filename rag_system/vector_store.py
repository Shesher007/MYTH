...
from myth_config import config, load_dotenv  # noqa: E402
...
class VectorStoreManager:
    ...
    def __init__(self, nvidia_api_key: Optional[str] = None):
        ...
        if not self.nvidia_api_key:
            logger.warning(
                "NVIDIA_API_KEY not found in rotation. RAG search will fail."
            )

        # Initialize embeddings using NVIDIA NIM (Exclusive)
        logger.info(
            f"🧬 [VECTOR] Initializing NVIDIA embeddings with model: {self.model_name}"
        )

        self.embeddings = NVIDIAEmbeddings(model=self.model_name)
        logger.info("✅ [VECTOR] NVIDIA Neural Matrix Online (Qdrant)")

        # Initialize document processor
        self.document_processor = DocumentProcessor(nvidia_api_key=self.nvidia_api_key)
    ...
    async def add_documents(
        ...
        try:
            ...
            if not documents:
                return {
                    "success": False,
                    "error": "No non-empty documents extracted after filtering",
                }
            ...
            for i, doc in enumerate(documents):
                ...
                doc_id = hashlib.md5(
                    f"{file_hash}_{chunk_id}_{now_ts}".encode()
                ).hexdigest()
                ids.append(doc_id)
    ...
    async def add_langchain_documents(
        self,
        collection_name: str,
    ...
    async def hybrid_search(
        self, collection_name: str, query: str, k: int = 10, vector_weight: float = 0.7
    ) -> List[Dict]:
            ...
            for res in vector_results:
                ...
                for kw in keywords:
                    if kw in content_lower:
                        boost += 0.05
                    if kw in meta.get("file_name", "").lower():
                        boost += 0.1

                # Final fused score
                res["relevance_score"] = (score * vector_weight) + (
        ...
        try:
            ...
            if not file_paths:
                return {
                    "success": True,
                    "documents_added": 0,
                    "message": "No supported files found after filtering",
                }

            logger.info(
                f"📁 [INGEST] Found {len(file_paths)} relevant files. Processing in batches..."
            )
            ...
            for i in range(0, len(file_paths), batch_size):
                ...
                await asyncio.to_thread(
                    vector_store.add_documents, documents=documents, ids=batch_ids
                )
                total_chunks += len(documents)
                logger.info(
                    f"⚡ [INGEST] Batch {i // batch_size + 1} complete. Total chunks: {total_chunks}"
                )
