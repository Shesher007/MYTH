...
class RAGEvaluator:
    ...
    async def evaluate_query(self, collection: str, question: str) -> Dict[str, Any]:
        ...
        # 1. Get RAG Response
        response = await self.rag.create_qa_chain(collection).ainvoke({"question": question})
        ...
        """
        faithfulness_res = await self.eval_llm.ainvoke([HumanMessage(content=faithfulness_prompt)])
        ...
        Format: Score: [score] | Justification: [justification]
        """
        relevance_res = await self.eval_llm.ainvoke([HumanMessage(content=relevance_prompt)])
        ...
        Format: Score: [score] | Justification: [justification]
        """
        precision_res = await self.eval_llm.ainvoke([HumanMessage(content=precision_prompt)])
