...
class FolderProcessor:
    ...
    def should_ignore(self, file_path: Path) -> bool:
        ...
        try:
            if file_path.is_file() and file_path.stat().st_size > self.max_file_size:
                logger.warning(
                    f"Ignoring large file: {file_path} ({file_path.stat().st_size} bytes)"
                )
                return True
        except Exception:
            pass
    ...
    def scan_folder(self, folder_path: str, recursive: bool = True) -> Dict[str, Any]:
        ...
        for root, dirs, files in os.walk(folder):
            ...
            if len(rel_path.parts) >= self.max_depth:
                logger.warning(
                    f"⚠️ [FOLDER] Max depth reached at {root}. Skipping subdirectories."
                )
                dirs[:] = []
                continue
            ...
            for file_name in files:
                ...
                try:
                    ...
                    file_type_info = self.universal_processor.detect_file_type(
                        str(file_path)
                    )
                    category = file_type_info.get("category", "unknown")
                    extension = file_type_info.get(
    ...
    async def aprocess_folder(
        ...
        files_to_process = [
            ...
            if (not skip_categories or f["category"] not in skip_categories)
            and (process_archives or f["category"] != "archive")
            and (not extract_text_only or f["category"] != "executable")
        ]
        ...
        async def process_with_limit(file_info, semaphore):
            async with semaphore:
                return await self._process_single_file(
                    file_info["path"], collection_name, extract_text_only
                )
            ...
            try:
                ...
                if res["success"]:
                    ...
                    results["failed_files"].append(
                        {"file": res.get("file"), "error": res.get("error")}
                    )
            except Exception as e:
    ...
    async def _process_single_file(
        self, file_path: str, collection_name: str, extract_text_only: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.vector_store:
                try:
                    ...
                    logger.error(
                        f"Failed to add to vector store during folder process: {e}"
                    )
    ...
    async def _process_archive_file(
        self, archive_path: str, collection_name: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for extracted_file in extract_result.get("files", []):
                if os.path.isfile(extracted_file):
                    ...
                    file_result = await self._process_single_file(
                        extracted_file, collection_name
                    )
                    ...
                    if file_result["success"]:
                        ...
                        failed_files.append(
                            {"file": extracted_file, "error": file_result.get("error")}
                        )
            ...
            return {"success": False, "error": str(e), "archive": archive_path}

    def _extract_text_content(self, file_path: str, max_length: int = 10000) -> str:
        """
    ...
    def create_folder_summary(self, folder_path: str) -> Dict[str, Any]:
        ...
        sorted_by_size = sorted(
            scan_result["files"], key=lambda x: x["size"], reverse=True
        )[:10]
        ...
        sorted_by_mtime = sorted(
            scan_result["files"], key=lambda x: x["modified"], reverse=True
        )[:10]
