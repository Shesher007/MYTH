...
class RAGChain:
    ...
    def __init__(
        ...
        self.system_prompt = system_prompt or agent_config.prompts.get_full_system_prompt(category="complex")
        
        # Add RAG-specific injection
        self.system_prompt += "\n\n**RAG PROTOCOLS:**\n"
        self.system_prompt += "1. PRIORITIZE retrieved context over internal training data.\n"
        self.system_prompt += "2. CITE sources by filename and index (e.g. [audit_log.csv:34]).\n"
    ...
    def create_qa_chain(self, collection_name: str):
        ...
        rewrite_prompt = ChatPromptTemplate.from_messages([
            ("system", "Rewrite the following question as a robust standalone query for a search engine."),
            MessagesPlaceholder("chat_history"),
            ("human", "{question}"),
        ])

        rewrite_chain = rewrite_prompt | self.llm | RunnableLambda(lambda x: x.content if hasattr(x, "content") else str(x))

        # 2. Hybrid Retrieval
        async def retrieve_docs(question):
            results = await self.vector_store.hybrid_search(collection_name, question, k=20)
            return [
        ...
        async def rerank_docs(input_data):
            ...
            context_snippet = "\n".join([f"ID:{i} | Content:{d.page_content[:400]}" for i, d in enumerate(docs[:15])])
            prompt = (
        ...
        def format_docs(docs):
            ...
            for i, d in enumerate(docs):
                source = d.metadata.get("file_name") or d.metadata.get("file_path", "Unknown Source")
        ...
        rag_chain = (
            ...
            | RunnablePassthrough.assign(
                docs=RunnableLambda(wrap_retrieve)
            )
            | RunnablePassthrough.assign(
                reranked_docs=RunnableLambda(wrap_rerank)
            )
            | RunnablePassthrough.assign(
    ...
    async def query_with_sources(self, collection_name: str, query: str, k: int = 5) -> Dict[str, Any]:
        """Perform a single query and return sources (Async)"""
        results = await self.vector_store.hybrid_search(collection_name, query, k=k)
        
        if not results:
            return {"answer": "No relevant data found.", "sources": [], "confidence": 0.0}

        context = "\n\n".join([f"[{i}] {r['metadata'].get('file_name', '??')}:\n{r['content'][:600]}" for i, r in enumerate(results)])
    ...
    async def cross_document_analysis(self, query: str, collection_names: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        context = "\n\n".join([f"[{r['collection']}] {r['metadata'].get('file_name', '')}: {r['content'][:400]}" for r in all_results[:8]])
