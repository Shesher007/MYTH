...
class ArchiveExtractor:
    ...
    def _is_safe_path(self, base_dir: Path, target_path: Path) -> bool:
        ...
        try:
            ...
            return target_path.relative_to(base_dir) is not None
        except ValueError:
            return False
    ...
    def extract_archive(self, archive_path: str, 
        ...
        if current_depth >= self.max_depth:
            return {"success": False, "error": f"Maximum extraction depth reached ({self.max_depth})"}
    ...
    def _extract_zip(self, path: Path, extract_to: Path, 
        ...
        with zipfile.ZipFile(path, 'r') as zip_ref:
            ...
            for info in zip_ref.infolist():
                ...
                if total_files > self.max_files:
                    raise Exception(f"Archive contains too many files (> {self.max_files})")
                
                if total_size > (self.max_decompressed_size_mb * 1024 * 1024):
                    raise Exception(f"Archive decompressed size exceeds limit ({self.max_decompressed_size_mb} MB)")
                
                if info.compress_size > 0:
                    ratio = info.file_size / info.compress_size
                    if ratio > self.max_compression_ratio:
                        raise Exception(f"Suspicious compression ratio detected ({ratio:.1f}x) in {info.filename}")
    ...
    def _extract_7z(self, path: Path, extract_to: Path, 
                   password: Optional[str]) -> List[str]:
    ...
    def _extract_rar(self, path: Path, extract_to: Path, 
                    password: Optional[str]) -> List[str]:
        ...
        with rarfile.RarFile(path, 'r') as rar:
            ...
            for file_info in rar.infolist():
                ...
                if not self._is_safe_path(extract_to, target_path):
                    logger.warning(f"⚠️ Blocked unsafe file in RAR: {file_info.filename}")
                    continue
